###############################################################################
# biomass_slope_stability.R
#
# 4-panel figure (top-10% RMSE ensemble, 1900-2010), produced for five
# sliding window sizes: 3-year, 5-year, 10-year, 12-year, and 15-year.
...
#
# Reference lines: -1 SD of the ensemble-mean unexploited metric in
#   1901-1929. Reference uses the mean across sims per year first
#   (removing parameter uncertainty), then SD over the 29-year period
#   (pure temporal variability). All signals move negative when fishing
...
#
# Outputs:
#   biomass_slope_stability_3yr_top10pct.png
#   biomass_slope_stability_5yr_top10pct.png
#   biomass_slope_stability_10yr_top10pct.png
#   biomass_slope_stability_12yr_top10pct.png
#   biomass_slope_stability_15yr_top10pct.png
###############################################################################
...
# Total window = win_before + 1 + win_after
WINDOW_CONFIGS <- list(
...
# Reference: -CV of the ensemble-mean unexploited biomass in 1901-1929.
# Mean across sims per year first, then CV of 29-year series (temporal only).
clim_yr_mean <- total_clim %>%
  filter(Year %in% 1901:1929) %>%
  group_by(Year) %>%
  summarise(mean_g = mean(total_g, na.rm = TRUE), .groups = "drop")
ref_delta_bm <- -(sd(clim_yr_mean$mean_g) / mean(clim_yr_mean$mean_g))
rm(clim_yr_mean)
...
# Reference: -1 SD of ensemble-mean unexploited slope in 1901-1929 (temporal only)
slope_yr_mean <- all_slopes %>%
  filter(ensemble == "Unexploited", time %in% 1901:1929) %>%
  group_by(time) %>%
  summarise(mean_slope = mean(slope, na.rm = TRUE), .groups = "drop")
ref_delta_slope <- -sd(slope_yr_mean$mean_slope, na.rm = TRUE)
rm(slope_yr_mean)
...
for (cfg in WINDOW_CONFIGS) {
  ...
  # Reference: -1 SD of ensemble-mean unexploited 1/CV in 1901-1929 (temporal only)
  inv_cv_clim_yr_mean <- inv_cv_clim %>%
    filter(Year %in% 1901:1929) %>%
    group_by(Year) %>%
    summarise(mean_inv_cv = mean(inv_cv, na.rm = TRUE), .groups = "drop")
  ref_delta_inv_cv <- -sd(inv_cv_clim_yr_mean$mean_inv_cv, na.rm = TRUE)
  rm(inv_cv_clim_yr_mean, inv_cv_fish, inv_cv_clim)
  ...
  # Reference: -1 SD of ensemble-mean unexploited 1/slope_SD in 1901-1929 (temporal only)
  inv_sd_yr_mean <- inv_sd_per_sim %>%
    filter(ensemble == "Unexploited", time %in% 1901:1929) %>%
    group_by(time) %>%
    summarise(mean_inv_sd = mean(inv_sd_slope, na.rm = TRUE), .groups = "drop")
  ref_delta_inv_slope_sd <- -sd(inv_sd_yr_mean$mean_inv_sd, na.rm = TRUE)
  rm(inv_sd_yr_mean, inv_sd_per_sim)
  ...
  outfile <- sprintf("biomass_slope_stability_%s_top10pct.png", win_label)
  ggsave(outfile, p_out, width = 10, height = 16, dpi = 300)
  message(sprintf("Saved: %s", outfile))

  rm(delta_inv_cv_summary, delta_inv_slope_sd_summary,
