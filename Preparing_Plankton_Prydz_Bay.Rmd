...
df_area <- GFDL_diat_long$area_m2
df_date <- GFDL_diat_long$date

GFDL_diat_long <- GFDL_diat_long %>% # Create a tidy date variable
...
# GFDL_pico_long_annual <- GFDL_pico_long[,-1] %>%
#   group_by(year = lubridate::floor_date(date, "year")) %>%
#     summarize(total_C_mol = mean(total_C_mol))
...
GFDL_diat_long_annual <- GFDL_diat_long[,-1] %>%
  group_by(year = lubridate::floor_date(date, "year")) %>%
    summarize(total_C_mol = mean(total_C_mol))
...
GFDL_diaz_long <- GFDL_diaz_raw %>%
  ...
  group_by(variable) %>% # group by month
  summarise(total_C_mol = sum(C_g)) %>%  # sum the carbon across all grid cells by each month
  mutate(date = seq(as.Date("1961-01-01"),as.Date("2010-12-01"),by="months"))


GFDL_diaz_long_annual <- GFDL_diaz_long[,-1] %>%
  group_by(year = lubridate::floor_date(date, "year")) %>%
    summarize(total_C_mol = mean(total_C_mol))
...
GFDL_zmicro_long <- GFDL_zmicro_raw %>%
  ...
  mutate(C_g = C_g_m2*422292280) %>% # multiply the m^2 value by the grid cell area to get total C per grid cell
  group_by(variable) %>% # group by month
  summarise(total_C_g = sum(C_g)) %>%  # sum the carbon across all grid cells by each month
  mutate(total_C_g_m2 = total_C_g/total_area) %>%  # sum the carbon across all grid cells by each month
  mutate(date = seq(as.Date("1961-01-01"),as.Date("2010-12-01"),by="months"))


GFDL_zmicro_long_annual <- GFDL_zmicro_long[,-1] %>%
  group_by(year = lubridate::floor_date(date, "year")) %>%
    summarize(total_C_mol = mean(total_C_mol))
...
GFDL_zmeso_long_annual <- GFDL_zmeso_long[,-1] %>%
  group_by(year = lubridate::floor_date(date, "year")) %>%
    summarize(total_C_mol = mean(total_C_mol))
...
# Convert total carbon to gww and then get numerical abundance by dividing by size class mid point
# This step assumes that all plankton are the midpoint size
pico_abund_annual <- GFDL_pico_long_annual[,2]*10/pico_mid
large_abund_annual <- (GFDL_diat_long_annual[,2] + GFDL_diaz_long_annual[,2])*10/large_mid
micro_abund_annual <- GFDL_zmicro_long_annual[,2]*10/micro_mid
meso_abund_annual <- GFDL_zmeso_long_annual[,2]*10/meso_mid
...
# Save
# write.table(out_isimip, file = "GFDL_resource_spectra_annual.dat", quote = FALSE, row.names = TRUE, col.names = TRUE)
# write.table(isimip_slope, file = "GFDL_resource_slope_annual.dat", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
# write.table(isimip_intercept, file = "GFDL_resource_intercept_annual.dat", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
```

```{r}
isimip_slope <- read.table("FishMIP_Plankton_Forcing/GFDL_resource_slope_annual.dat")
```


```{r}
years <- seq(as.Date("1961-01-01"),as.Date("2010-12-01"),by="year") # create a tidy date variable
...
# ggsave("plots/GFDL_ISIMIP3a_Resource_Slope_Prydz_Bay.tiff", plot = p_slope, units="cm", width=20, height=16, dpi =300)
```

```{r}
isimip_intercept <- read.table("FishMIP_Plankton_Forcing/GFDL_resource_intercept_annual.dat")
```
...
p_intercept <- ggplot(,aes(x = years, y = isimip_intercept[,1])) +
  ...
  geom_smooth(colour="black", alpha = 0.2) +
  xlab("Year") +
  ylab("Background Resource Intercept") +
  theme_classic()

ggsave("plots/GFDL_ISIMIP3a_Resource_Intercept_Prydz_Bay.tiff", plot = p_intercept, units="cm", width=20, height=16, dpi =300)
```
...
# write.table(out_isimip, file = "FishMIP_Plankton_Forcing/GFDL_resource_spectra_w_full_164.dat", quote = FALSE, row.names = TRUE, col.names = TRUE)
# write.table(isimip_slope, file = "FishMIP_Plankton_Forcing/GFDL_resource_slope_w_full_164.dat", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
# write.table(isimip_intercept, file = "FishMIP_Plankton_Forcing/GFDL_resource_intercept_w_full_164.dat", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
