###############################################################################
# TEST NEW METRICS — 20-Simulation Validation
#
# Purpose:
#   Runs 20 paired simulations through the updated ecosystem assessment pipeline
#   to fully validate empirical thresholds and all output figures:
#
#   Modified:
...
#
# Usage:
#   Rscript test_new_metrics_20sims.R
###############################################################################

cat("=============================================================\n")
cat("TEST RUN: New Metric Validation (20 simulations)\n")
cat("=============================================================\n\n")
...
for (col in expected_new) {
  ...
  if (!is.null(th) && !is.na(th$median)) {
    ...
    n_thresholds_derived <- n_thresholds_derived + 1
  } else {
    cat(sprintf("  [FAIL] %-30s: threshold not derived (NULL or NA)\n", col))
  }
}
cat(sprintf("\n  Empirical thresholds derived for new metrics: %d / %d\n",
            n_thresholds_derived, length(expected_new)))
if (n_thresholds_derived == length(expected_new)) {
  cat("  [PASS] All new metric thresholds derived successfully!\n")
} else {
  cat("  [WARN] Some thresholds missing — check B0 reference count\n")
}
...
# Print structural deviation heatmap info
...
cat(sprintf("  Full pipeline (%d sims):    COMPLETED\n", N_TEST))
...
cat(sprintf("  Empirical thresholds:       %d/%d new metrics have thresholds\n",
            n_thresholds_derived, length(expected_new)))
