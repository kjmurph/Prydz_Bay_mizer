...
#
# Run from the repository root. Reads inputs from "Manuscript data/" and writes
# figures to "Manuscript figures/":
#   yield_stacked_top10pct_sd_errorbars.png   (yield panel, standalone)
...
#
# Required files in "Manuscript data/":
#   yield_cached_top10pct_stack.rds, yield_cached_top10pct_total_per_sim.rds,
#   yield_cached_obs_stack.rds, yield_observed_timeseries.csv,
#   yield_rmse_per_sim.csv, params_sel_adj.rds,
#   spectra_cache_ref_period.rds, spectra_cache_species_ref_period.rds
#
...
# Manuscript bundle paths: inputs read from "Manuscript data/", figures written
# to "Manuscript figures/". Run from the repository root.
...
stack_cache <- file.path(DATA_DIR, sprintf("yield_cached_top%dpct_stack.rds", PCT))          # mean/sp/yr
total_cache <- file.path(DATA_DIR, sprintf("yield_cached_top%dpct_total_per_sim.rds", PCT))  # total/yr/sim
obs_cache   <- file.path(DATA_DIR, "yield_cached_obs_stack.rds")
for (f in c(stack_cache, total_cache, obs_cache))
  if (!file.exists(f)) stop("Missing cache in Manuscript data/: ", f)
...
obs_raw_cols   <- read.csv(file.path(DATA_DIR, "yield_observed_timeseries.csv"), nrows = 1)
...
spectra_cache_path <- file.path(DATA_DIR, "spectra_cache_ref_period.rds")
species_cache_path <- file.path(DATA_DIR, "spectra_cache_species_ref_period.rds")
temporal_cache_path <- file.path(DATA_DIR,
  "climate_only_community_spectrum_by_year_top10pct.rds")
params_path        <- file.path(DATA_DIR, "params_sel_adj.rds")
rmse_path          <- file.path(DATA_DIR, "yield_rmse_per_sim.csv")
...
cat("  Ratio panel built (embedded in combined figures).\n\n")
