# =============================================================================
# FIGURE 1 (ANNUAL MEAN-MAXIMUM-SIZE, LINEAR AXIS, + DOMAIN MAP INSET)
...
#   B  Mean maximum body mass of the observed catch, ANNUAL (no smoothing), on a
#      LINEAR axis
#
# EXACTLY F01_figure1_mmw_smooth3_map_rebuilt167.R WITH TWO CHANGES TO PANEL B
# (Kieran, 2026-08-11): the 3-year window is off, and the y axis is linear.
# Panel A, the map, the inset key, the layout and the figure size are identical.
#
# WHAT THE LINEAR AXIS DOES TO THIS SERIES, because it is not a neutral choice.
# The indicator spans 6.8 decades (1.65e-5 to 103 t). A linear 0-108 t axis
# resolves the whaling era, where the series sits at ~100 t, and compresses
# EVERYTHING after 1970 -- the whole minke/krill era, 5-13 t -- into the bottom
# tenth of the panel. The three near-zero-catch years (1971, 1988, 1993) plot on
# the axis floor, no longer distinguishable from each other or from zero, and
# five of the nine species reference lines collapse onto that floor with them
# (REF_MIN_FRAC below). The log build carries the same numbers with the modern
# era legible; this one carries the magnitude of the fall. Neither is wrong --
# and the era medians (96.8 -> 5.87 t, 16.5x) are identical either way, because
# they come from the data, not the axis.
#
# The trend printed here is LINEAR, in t per decade, to match the axis; the log
# build's -0.285 log10 units per decade is the comparable figure there.
#
...
cat("\n=== mean maximum body mass of the catch, annual, linear axis ===\n")
...
png_out <- guard(file.path(FIGS, sprintf("fig1_mmw_linear_map_%s.png", SUF)))
pdf_out <- guard(file.path(FIGS, sprintf("fig1_mmw_linear_map_%s.pdf", SUF)))
...
write.csv(MMW, guard(file.path(DATA,
          sprintf("fig1_mmw_linear_map_series_%s.csv", SUF))), row.names = FALSE)
cat("\nWrote:\n  ", png_out, "\n  ", pdf_out, "\n")