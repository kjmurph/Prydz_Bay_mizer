# =============================================================================
# FIGURE 1 (ANNUAL GEOMETRIC MEAN-MAXIMUM-SIZE, LINEAR AXIS, + DOMAIN MAP INSET)
...
#
# F01_figure1_mmw_linear_map_rebuilt167.R with the arithmetic mean replaced by
# the geometric one (Kieran, 2026-08-11):
#
#     GM(y) = exp( sum_i C_iy * ln(w_max,i) / sum_i C_iy )
#
# Same catch-biomass weights; only the averaging scale changes. Panel A, the
# map, the inset key, the layout and the figure size are identical.
#
# READ THIS BEFORE USING THE FIGURE -- the two changes compound badly.
#
# The geometric mean is the noisier of the two constructions, as the original
# mean-maximum-mass header records: it spans 7.39 decades against the
# arithmetic's 6.80, because it lets a small krill tonnage pull the mean down
# through several decades of mass, where the arithmetic mean stays with whatever
# carries the tonnage. Its era medians are 91.9 -> 0.57 t, a 161-fold fall
# against the arithmetic's 16.5-fold; that difference is the construction, not
# the fishery.
#
# A LINEAR AXIS THEN HIDES ALMOST ALL OF IT. On 0-108 t, 20 of the 74 years with
# catch fall below 1% of the axis -- effectively the entire post-1971 record sits
# on the floor, indistinguishable from zero and from each other, along with five
# of the nine species reference lines (REF_MIN_FRAC below). The panel reduces to
# "~100 t until 1970, then nothing", which is true but is most of the signal
# thrown away.
#
# The arithmetic-on-linear sibling keeps 70 of 74 years off the floor, and the
# log builds keep all of them. If the geometric mean is the quantity of
# interest, it wants a log axis.
#
# The rest of this header is inherited and still applies except where panel B is
# concerned; notes 2 and 3 below describe the smoothed build and are kept for
# the map and layout decisions they also cover.
#
# ------------------------------------------------------------------------------
...
# The standalone map is NOT re-saved here: it is identical to the one
...
# GEOMETRIC mean: the catch-biomass-weighted mean of log(w_max), exponentiated.
# The weights are the same catch tonnages as the arithmetic build -- only the
# averaging scale changes.
num <- as.vector(C %*% log(wmax[spp])); den <- rowSums(C)

# --- NO SMOOTHING: the annual series ------------------------------------------
# `den > 0`: a year with no reported catch leaves the ratio undefined, so it is
# NA and the line breaks there -- 1941-45 and 2005/2009/2011/2012, exactly panel
# A's gaps.
MMW <- data.frame(Year = obs_raw$Year,
                  total_kt = den / 1e9,
                  mmw_t    = ifelse(den > 0, exp(num / den), NA) / 1e6) %>%
  filter(Year >= X_LIM[1], Year <= X_LIM[2])
...
fit <- lm(mmw_t ~ Year, data = MMW[k, ])     # LINEAR trend, to match the axis
cat("\n=== GEOMETRIC mean maximum body mass of the catch, annual, linear axis ===\n")
cat(sprintf("  1930-1970 median  %.3g t\n", sm(MMW$mmw_t, e1)))
cat(sprintf("  1971-2010 median  %.3g t  (%.1fx)\n", sm(MMW$mmw_t, e2),
            sm(MMW$mmw_t, e1) / sm(MMW$mmw_t, e2)))
cat(sprintf("  trend             %+.2f t per decade (p = %.2g, R2 = %.2f)\n",
...
cat(sprintf("  years below 1%% of the axis (1.05 t): %d of %d\n",
            sum(MMW$mmw_t < 1.05, na.rm = TRUE), sum(is.finite(MMW$mmw_t))))
...
# ON A LINEAR AXIS THE REFERENCE LINES ARE FILTERED BY SEPARABILITY, NOT RANGE.
# All nine w_max values lie inside 0-105 t, but the bottom five span 0.158 t down
# to 4.2 g and would be drawn on top of each other along the axis floor, three of
# them within a pixel of it. The rule is an explicit fraction of the upper limit
# (REF_MIN_FRAC), applied to the data rather than hand-listed, so it stays
# correct if the limit moves; at 1% it keeps the same four the log build's
# tightened axis kept.
y_lim <- c(0, 108)
REF_MIN_FRAC <- 0.01
ref <- ref_all %>% filter(y >= REF_MIN_FRAC * y_lim[2], y <= y_lim[2])
cat("\n=== panel B reference lines ===\n")
cat("  drawn  :", paste(ref$lab, collapse = ", "), "\n")
cat(sprintf("  dropped (below %.0f%% of the axis, indistinguishable from zero):\n",
            100 * REF_MIN_FRAC))
...
pB <- ggplot(MMW, aes(Year, mmw_t)) +
  ...
  # No in-panel text: one series, and the construction belongs in the caption.
  scale_colour_manual(values = sp_cols, guide = "none") +
  scale_x_continuous(breaks = X_BRK, limits = X_LIM) +
  scale_y_continuous(breaks = seq(0, 100, 25)) +
  coord_cartesian(ylim = y_lim) +
  labs(x = "Year",
       y = expression("Geometric mean maximum body mass of catch, "*w[max]*" (t)"),
       tag = "B") +
  theme_classic(base_size = 11) +
...
png_out <- guard(file.path(FIGS, sprintf("fig1_mmwgeo_linear_map_%s.png", SUF)))
pdf_out <- guard(file.path(FIGS, sprintf("fig1_mmwgeo_linear_map_%s.pdf", SUF)))
...
write.csv(MMW, guard(file.path(DATA,
          sprintf("fig1_mmwgeo_linear_map_series_%s.csv", SUF))), row.names = FALSE)
cat("\nWrote:\n  ", png_out, "\n  ", pdf_out, "\n")