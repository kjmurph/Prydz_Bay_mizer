...
# -- SD-ribbon variant with 1929 x-extent and a small negative y-floor beneath 0.
#
...
#      smoothing), with a geometric-mean SD ribbon on a LINEAR axis
...
#
# Writes Manuscript figures/fig1_mmwgeo_mti_linear_map_rebuilt167_sd_ribbon_1929extent_zero_buffer.{png,pdf}
#        Manuscript data/fig1_mmwgeo_mti_linear_map_series_rebuilt167_sd_ribbon_1929extent_zero_buffer.csv
# =============================================================================
...
# ----------------------------------------------------------- PANEL B --------
# GEOMETRIC mean maximum body mass of catch, using the same log-space SD ribbon
# as the geometric mean requires, while keeping a small negative floor below 0.
...
Y_FLOOR <- -2
Y_CEIL <- max(MMW$mmw_hi, na.rm = TRUE) * 1.12
y_lim <- c(Y_FLOOR, Y_CEIL)
MMW_ribbon <- MMW %>%
  mutate(ribbon_lo = pmax(mmw_lo, Y_FLOOR),
         ribbon_hi = pmin(pmax(mmw_hi, Y_FLOOR), y_lim[2]))
y_breaks <- seq(0, ceiling(y_lim[2] / 25) * 25, 25)

pB <- ggplot(MMW, aes(Year)) +
  geom_ribbon(data = MMW_ribbon, aes(ymin = ribbon_lo, ymax = ribbon_hi),
              fill = "grey40", alpha = 0.18, na.rm = TRUE) +
  geom_vline(xintercept = c(whal, krl), linetype = "dashed", colour = "grey45") +
...
png_out <- guard(file.path(FIGS, "fig1_mmwgeo_mti_linear_map_rebuilt167_sd_ribbon_1929extent_zero_buffer_final_2.png"))
pdf_out <- guard(file.path(FIGS, "fig1_mmwgeo_mti_linear_map_rebuilt167_sd_ribbon_1929extent_zero_buffer_final_2.pdf"))
csv_out <- guard(file.path(DATA, "fig1_mmwgeo_mti_linear_map_series_rebuilt167_sd_ribbon_1929extent_zero_buffer_final_2.csv"))
