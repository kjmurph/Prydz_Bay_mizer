...
# -- second version, no ribbon, 1929 x-extent with 1930 first tick.
#
...
#
# Writes Manuscript figures/fig1_mmwgeo_mti_linear_map_rebuilt167_no_ribbon_1929extent.{png,pdf}
#        Manuscript data/fig1_mmwgeo_mti_linear_map_series_rebuilt167_no_ribbon_1929extent.csv
# =============================================================================
...
pB <- ggplot(MMW, aes(Year)) +
  geom_vline(xintercept = c(whal, krl), linetype = "dashed", colour = "grey45") +
  geom_point(aes(y = mmw_t), size = 1.4, colour = "grey20", na.rm = TRUE) +
...
sil_anchor <- data.frame(
  ...
  x = c(1932, 1939, 1952, 1970, 2008, 2008, 2008, 2008),
  y = c(12, 30, 47, 64, 86, 96, 108, 122),
  fill = c("#F8766D", "#93AA00", "#E8B33C", "#00C19F",
           "#00B9E3", "#619CFF", "#DB72FB", "#FF61C3"),
  height = c(8, 11, 12, 14, 18, 20, 25, 31),
  stringsAsFactors = FALSE) %>%
  filter(species %in% caught)

sil_anchor$img <- I(lapply(sil_anchor$species, function(nm) {
  key <- gsub("[^a-z0-9]+", "_", tolower(nm))
  f <- file.path("R/figures/fig1/phylopic_cache", paste0(key, ".rds"))
  if (!file.exists(f)) return(NULL)
  readRDS(f)$img
}))

for (i in seq_len(nrow(sil_anchor))) {
  if (is.null(sil_anchor$img[[i]])) next
  pB <- pB + rphylopic::geom_phylopic(
    data = data.frame(x = sil_anchor$x[i], y = sil_anchor$y[i],
                      img = I(list(sil_anchor$img[[i]])), stringsAsFactors = FALSE),
    aes(x = x, y = y, img = img),
    fill = sil_anchor$fill[i],
    height = sil_anchor$height[i],
    alpha = 0.95,
    inherit.aes = FALSE)
}
...
pC <- ggplot(MTI, aes(Year)) +
  ...
  geom_point(aes(y = mti), size = 1.3, colour = "grey30", na.rm = TRUE) +
...
png_out <- guard(file.path(FIGS, "fig1_mmwgeo_mti_linear_map_rebuilt167_no_ribbon_1929extent_exploited_silhouettes_v2.png"))
pdf_out <- guard(file.path(FIGS, "fig1_mmwgeo_mti_linear_map_rebuilt167_no_ribbon_1929extent_exploited_silhouettes_v2.pdf"))
csv_out <- guard(file.path(DATA, "fig1_mmwgeo_mti_linear_map_series_rebuilt167_no_ribbon_1929extent_exploited_silhouettes_v2.csv"))
