...
#
# Generates four figure versions for EVERY biomass and structural metric:
...
#   4. Ensemble with single ribbon (50% CI only), 1900 onward
#
# Features:
#   - Species/metric-specific peak-effort vertical lines
#   - Onset of whaling line on all plots
...
OUTPUT_DIR <- "ecosystem_assessment_outputs/figure_variants"
...
# =============================================================================
# Onset of industrial whaling — shown on ALL plots
WHALING_ONSET <- list(year = 1904, label = "Whaling onset",
                      color = "grey45", linetype = "dotted")

# Species/metric-specific peak effort events
# Each entry: year, label, color, linetype
# Adjust years/labels to match your preferred periodisation
PEAK_EVENTS <- list(

  baleen_biomass = list(
    list(year = 1937, label = "Peak blue/fin catches",
         color = "#1f78b4", linetype = "dashed"),
    list(year = 1964, label = "IWC blue whale moratorium",
         color = "#a6cee3", linetype = "dashed")
  ),

  whale_biomass = list(
    list(year = 1937, label = "Peak baleen catches",
         color = "#1f78b4", linetype = "dashed"),
    list(year = 1964, label = "Peak sperm whale catches",
  ...
  fish_biomass = list(
    list(year = 1997, label = "Peak IUU toothfish",
         color = "#ff7f00", linetype = "dashed")
  ),

  krill_biomass = list(
    list(year = 1982, label = "Peak krill catch",
...
SILHOUETTES <- list(
  baleen_biomass = list(
    ...
    path = "C:\Users\kjmurphy\OneDrive - University of Tasmania\Sizing The Southern Ocean\silhouettes\whale.png",
  ...
  apex_biomass = list(
    ...
    path = "C:\Users\kjmurphy\OneDrive - University of Tasmania\Sizing The Southern Ocean\silhouettes\orca.png",
    x = 0.88,
    y = 0.85,
    width = 0.18,
    alpha = 0.3
...
get_events_for_metric <- function(metric_col, metric_type = "biomass") {
  # Always include whaling onset
  events <- list(WHALING_ONSET)
  ...
  # Filter to post-YEAR_MIN
  events <- events[sapply(events, function(e) e$year >= YEAR_MIN)]
  return(events)
}
...
add_silhouette <- function(p, sil_data) {
  ...
  # Use annotation_custom with rasterGrob
  grob <- rasterGrob(
    sil_data$img,
    interpolate = TRUE,
  ...
  p + annotation_custom(
    ...
    # Override: place via coord system using viewport trick
    # annotation_custom doesn't support normalised coords well,
    # so we use a patchwork inset approach instead
    inset_element(
      grob, left = sil_data$x - sil_data$width / 2,
...
build_structural_ensemble <- function(plot_data, b0_env, events,
  ...
  # B0 envelope (structural uses wider envelope from empirical thresholds)
  if (!is.na(b0_env$q05)) {
    p <- p +
      annotate("rect", xmin = -Inf, xmax = Inf,
...
save_variant <- function(plot_obj, base_name, variant_suffix,
  ...
  pdf_path <- file.path(OUTPUT_DIR, paste0(base_name, "_", variant_suffix, ".pdf"))
...
generate_biomass_variants <- function(data, metric_col, config) {
  ...
  # Variant 4: Ensemble, 50% CI ribbon only
...
generate_all_figure_variants <- function() {
  ...
  cat("Category A — Biomass metrics (4 variants each):\n")
  ...
  cat("\nCategory B — Structural metrics (4 variants each):\n")
  ...
  n_total   <- (n_biomass + n_struct) * 4
  cat("\n=============================================================\n")
  cat("COMPLETE\n")
  cat(sprintf("  Biomass metrics:    %d x 4 variants = %d figures\n",
              n_biomass, n_biomass * 4))
  cat(sprintf("  Structural metrics: %d x 4 variants = %d figures\n",
              n_struct, n_struct * 4))
  cat(sprintf("  Total:              %d figures\n", n_total))
