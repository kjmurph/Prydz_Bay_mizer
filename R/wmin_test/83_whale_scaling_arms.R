...
# PHASE 83: matchGrowth at the CORRECT position -- after the erepro steady, and
# before the recap. Distinct from USE_MATCHGROWTH above, which is phase 61's
# in-ramp placement and stays off. Default OFF so earlier arms reproduce.
...
WH_TAG <- gsub("[^a-z]", "", paste(substr(trimws(strsplit(
  Sys.getenv("P83_WHALE_SPECIES", "baleen whales"), ",")[[1]]), 1, 3),
  collapse = ""))
STEM <- sprintf("83_wh%s%s_%s_n%d_%s%s_recap%s_K%s_%s", WH_TAG,
                if (MG_POST) paste0("_mgpost", MG_KEEP) else "",
...
# PHASE 77: the top N by the CUT-A METRIC, taken from the FULL post-refit
# ranking rather than from cut A itself. Cut A is the top 10% (167) of the 1,668
# accepted members under the unweighted pooled log10 yield RMSE; the same metric
# ranks deeper. VERIFIED: cut A IS exactly the top 167 of this ranking, so
# extending to 500 continues the same rule rather than substituting another.
# RMSE at rank 1 / 167 / 500 = 1.522 / 1.680 / 1.747.
...
cat(sprintf("ranking: %d members | taking top %d | RMSE %.4f .. %.4f\n",
...
# P83_DRAWS lets the build read a PRE-SUBSTITUTED draws file (phase 87) instead
# of the raw one. With P83_SUBSTITUTE=0 no further substitution is applied here,
# so the draws are exactly what is on disk and the run is reproducible from that
# file rather than from a random stream inside this script.
DRAWS_FILE <- Sys.getenv("P83_DRAWS", file.path(OUT_LARGE, "43_member_draws.rds"))
if (!file.exists(DRAWS_FILE)) stop("missing draws: ", DRAWS_FILE, call. = FALSE)
DR <- readRDS(DRAWS_FILE)
cat("draws:", basename(DRAWS_FILE),
    if (!is.null(DR$substitution))
      sprintf(" | PRE-SUBSTITUTED, seed %s, %d cells",
              DR$substitution$seed, nrow(DR$substitution$log)) else
      " | raw", "\n")
SUBSTITUTE <- Sys.getenv("P83_SUBSTITUTE", "1") == "1"
...
# matchGrowth needs an explicit age_mat column. Without one it runs
# set_species_param_default(sp, "age_mat", age_mat_vB(params)) and silently
# targets the k_vb PLACEHOLDER instead -- which for this model gives minke and
# sperm whales the SAME value, 16.832, against realised 21.48 and 34.20. That
# would decalibrate every member without erroring, so it is asserted, not
# assumed.
if (MG_POST) {
  ...
  cat(sprintf("  age target from the base's own age_mat column; factor on the ",
              ""))
  cat(sprintf("UNPERTURBED base spans %.4f-%.4f\n", min(f0), max(f0)))
  if (max(abs(f0 - 1)) > 0.05)
    cat("  NOTE: matchGrowth is NOT a no-op on the base -- factor differs from",
        "1 by more than 5%\n")
}
...
# --- how many ramp steps -----------------------------------------------------
# --- PHASE 83: whale groups are never scaled DOWN ----------------------------
# 60.8% of the usable phase-77 members draw baleen whales BELOW 1, which lowers
# their R_max and is why baleen stays pinned at reproduction level ~0.92 while
# sperm whales (median draw 3.25) came free. Members that reduce a treated whale
# group are given a RANDOM multiplier instead, uniform on [WH_MIN, WH_MAX].
#
# SEEDED AND RECORDED. The draw is made ONCE here, not inside the worker, so it
# is deterministic under P83_SEED, identical across arms for the same member,
# and written to the per-species table as whale_drawn / whale_used.
WH_SPECIES <- trimws(strsplit(Sys.getenv("P83_WHALE_SPECIES",
                                         "baleen whales"), ",")[[1]])
WH_SPECIES <- WH_SPECIES[nzchar(WH_SPECIES)]
WH_MIN  <- as.numeric(Sys.getenv("P83_WHALE_MIN", "5"))
WH_MAX  <- as.numeric(Sys.getenv("P83_WHALE_MAX", "50"))
WH_SEED <- as.integer(Sys.getenv("P83_SEED", "20260814"))
if (length(setdiff(WH_SPECIES, SPN)))
  stop("P83_WHALE_SPECIES not in the model: ",
       paste(setdiff(WH_SPECIES, SPN), collapse = ", "), call. = FALSE)

# lookup: [member, species] -> replacement multiplier. Drawn for EVERY member so
# the seed stream does not depend on which members happen to be below 1; only
# the members actually below 1 will use their value.
set.seed(WH_SEED)
WH_FIX <- matrix(runif(length(members) * length(WH_SPECIES), WH_MIN, WH_MAX),
                 nrow = length(members), ncol = length(WH_SPECIES),
                 dimnames = list(as.character(members), WH_SPECIES))
wh_below <- sapply(WH_SPECIES, function(s) sum(sapply(members, function(si)
  as.numeric(DR$draws[[as.character(si)]]$abundance_scaling[s]) < 1)))
cat("\nwhale scaling: ", paste(WH_SPECIES, collapse = ", "),
    " | replacement U(", WH_MIN, ", ", WH_MAX, ") | seed ", WH_SEED, "\n", sep = "")
for (s in WH_SPECIES)
  cat(sprintf("  %-16s %d of %d members draw below 1 and will be replaced\n",
              s, wh_below[[s]], length(members)))

# --- the mesozooplankton replacement value -----------------------------------
# In one dimension the GEOMETRIC MEDIAN coincides with the ordinary median, so
# both resolve to the same number; the median is already the outlier-robust
# choice here (the geometric MEAN of the same draws is 21.01, pulled upward by a
# 404,110 draw). Reported so the choice is visible rather than implicit.
MZ_ALL <- sapply(members, function(si)
  as.numeric(DR$draws[[as.character(si)]]$abundance_scaling["mesozooplankton"]))
MESO_FIX <- median(MZ_ALL[MZ_ALL > 1])
cat(sprintf("\nmesozooplankton draws: %d below 1, %d above | replacement %.4f\n",
            sum(MZ_ALL < 1), sum(MZ_ALL > 1), MESO_FIX))
cat(sprintf("  (geometric mean of the >1 draws %.3f, max %.4g -- median used)\n",
            exp(mean(log(MZ_ALL[MZ_ALL > 1]))), max(MZ_ALL)))
...
worker <- function(si) {
  ...
  # PHASE 77: MESOZOOPLANKTON IS NEVER SCALED DOWN. There is no hypothesis that
  # would justify a member drawing LESS mesozooplankton: it takes 79% of its
  # encounter from the prescribed resource, which is identical across members, so
  # a member-specific reduction is an artefact of applying one random draw to all
  # 19 groups. Any draw below 1 is replaced by MESO_FIX -- the median of the
  # draws ABOVE 1 across the selected members. Draws already above 1 are kept.
  mz_i <- match("mesozooplankton", SPN)
  sc_mz_drawn <- sc[mz_i]
  if (SUBSTITUTE && sc[mz_i] < 1) sc[mz_i] <- MESO_FIX

  # PHASE 83: the same rule for the treated whale groups, with a per-member
  # random replacement instead of a single median.
  ...
  # --- 2c. matchGrowth, AFTER the perturbation has settled -------------------
  # A DIFFERENT POSITION from phase 61's block above, which sits INSIDE the ramp
  # (before the erepro steady) and is the placement its own header documents as
  # the failure mode. Here it runs on the settled, perturbed state and BEFORE
  # the recap, because matchGrowth ends on setBevertonHolt(params), which
  # recomputes R_max and RESETS the reproduction level -- measured on the
  # reference, 0.9000 -> 0.9017. Capping first would simply be undone.
  #
  # keep = "biomass" preserves the biomass of the object AS PASSED IN, i.e. the
  # PERTURBED biomass, not the reference: verified, initial_n x5 comes out at
  # 5.014x the reference. It is not exact -- mizer captures the entry biomass
  # with `usecutoff` (a typo for `use_cutoff`, silently ignored, so full range)
  # and the exit biomass with `use_cutoff = TRUE`, giving ~0.2-0.3% drift per
  # call. Do not loop this without accounting for that.
  if (MG_POST) {
