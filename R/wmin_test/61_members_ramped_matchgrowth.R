# =============================================================================
# Phase 61 -- top-10 cut-A members on the phase-59 reference, with the GAMMA
# DRAW DROPPED and growth re-tuned by a RAMPED matchGrowth + steady loop
#
...
STEPS_REQ  <- as.integer(Sys.getenv("P61_STEPS", "10"))
MAX_STEP_X <- as.numeric(Sys.getenv("P61_MAX_STEP_X", "1.5"))
...
STEM <- sprintf("61_ramp_n%d_%s%s_K%s_%s", N,
...
if (mode == "run") {
  ...
  if (length(todo)) {
    ...
    clusterExport(cl, c("BASE", "DR", "SPN", "GAMMA0", "AGE_TGT", "K", "KEEP",
                        ...
                        "RECAP", "STEADY_TMAX", "RAMP_TMAX", "SPINUP_YEARS",
