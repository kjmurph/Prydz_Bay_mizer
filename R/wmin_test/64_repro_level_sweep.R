# =============================================================================
# Phase 64 -- REPRODUCTION LEVEL SWEEP for the marine mammal groups
#
# A copy of 61_members_ramped_matchgrowth.R. Phase 61 is NOT modified. The whole
# header below is inherited from it and still describes the shared protocol;
# THIS BLOCK states the only things that differ.
#
# ---------------------------------------------------------------- the question
# The six marine mammal groups are the six most reproduction-clamped groups in
# the model. Median reproduction level across the 122 stable phase-61 members:
# sperm 0.983 -> capped 0.900, baleen 0.948 -> 0.900, minke 0.924 -> 0.900,
# large divers 0.907 -> 0.900, medium divers 0.901 -> 0.900, orca 0.883
# (already under the cap). Every other group sits at 0.32-0.87.
#
# Recruitment elasticity to egg production is EXACTLY (1 - reproduction_level),
# so at 0.9 a HUNDRED-fold rise in egg production buys 11% more recruits:
#
#     level    x2 eggs   x5     x10    x100
#     0.90     1.05      1.09   1.10   1.11
#     0.25     1.60      2.50   3.08   3.88
#     0.10     1.82      3.57   5.26   9.17
#
# The reference model is calibrated to 2001-2010 biomass -- a DEPLETED state for
# the whales -- and the 0.9 cap re-pins that near-saturated level onto every
# member after the draws. The hypothesis under test is that this is why the
# unfished spin-up cannot build a pre-exploitation whale stock, and therefore
# why the observed baleen catch is 23.8x the calibrated stock.
#
# ------------------------------------------------------------- what changes
# 1. THE RECAP IS SPECIES-SELECTIVE. The six marine mammal groups are capped at
#    P64_TARGET; EVERY OTHER GROUP KEEPS THE 0.9 CAP. One variable versus the
#    production run, so any change in the food web is attributable.
# 2. THE STEM ENCODES the target and the scope, so sweep arms cannot overwrite
#    each other or the phase-61 output.
# 3. P64_STEPS AND P64_MAX_STEP_X DEFAULT TO 1 AND 1e9, giving K = 1.
#    THIS MATTERS: phase 61's own defaults (STEPS 10, MAX_STEP_X 1.5) give
#    K = 25, but the stored ensemble
#    61_ramp_n167_nomg_K1_reproduction_level was built with K = 1 -- verified
#    from its trace, one ramp row per member at t = 1. Re-running phase 61 at
#    its defaults would NOT reproduce it. These defaults do.
#
# THE CONTROL IS FREE: the same members already exist at RECAP 0.9 in
# 61_ramp_n167_nomg_K1_reproduction_level_states. Run this at P64_TARGET=0.9
# and it must reproduce those states; that is the pre-flight gate.
#
# ------------------------------------------------------------- the protocol
...
#
# USAGE  P64_TARGET=0.1 Rscript R/wmin_test/64_repro_level_sweep.R [run|collect|status]
# ENV    P64_TARGET (0.9)  reproduction level for the treated groups
#        P64_GROUPS (mammals|all)  which groups get P64_TARGET
#        P61_BASE, P61_N (10), P61_CORES (10), P61_STEPS (1),
#        P61_MAX_STEP_X (1e9), P61_MAX_STEPS (40), P61_RAMP_TOL (0.01),
...
FINAL_PRESERVE <- Sys.getenv("P61_FINAL_PRESERVE", "reproduction_level")
...
# Phase 64. RECAP above remains the cap for every UNTREATED group; RECAP_TARGET
# applies only to RECAP_GROUPS. With P64_TARGET = 0.9 the two are identical and
# this script must reproduce the phase-61 states exactly -- the pre-flight gate.
RECAP_TARGET <- as.numeric(Sys.getenv("P64_TARGET", "0.9"))
RECAP_GROUPS <- Sys.getenv("P64_GROUPS", "mammals")
MAMMALS <- c("minke whales", "orca", "sperm whales", "baleen whales",
             "medium divers", "large divers")
stopifnot(RECAP_GROUPS %in% c("mammals", "all"), is.finite(RECAP_TARGET),
          RECAP_TARGET > 0, RECAP_TARGET < 1)
...
STEM <- sprintf("64_rl%s_%s_n%d_%s%s_K%s_%s",
                gsub("\\.", "", format(RECAP_TARGET)), RECAP_GROUPS, N,
...
# --- phase 64: the per-species cap vector ------------------------------------
# Untreated groups keep RECAP (0.9). Treated groups get RECAP_TARGET. Built here
# rather than in the worker so the treated set is asserted once, loudly.
TREATED <- if (RECAP_GROUPS == "all") SPN else MAMMALS
if (length(setdiff(TREATED, SPN)))
  stop("treated groups not in the model: ",
       paste(setdiff(TREATED, SPN), collapse = ", "), call. = FALSE)
RECAP_VEC <- setNames(rep(RECAP, length(SPN)), SPN)
RECAP_VEC[TREATED] <- RECAP_TARGET
cat("\nreproduction treatment: ", length(TREATED), " group(s) at ",
    RECAP_TARGET, ", the other ", length(SPN) - length(TREATED), " at ", RECAP,
    "\n  treated: ", paste(TREATED, collapse = ", "), "\n", sep = "")
...
worker <- function(si) {
  ...
  if (is.finite(RECAP)) {
    p2 <- try(suppressWarnings(
      setBevertonHolt(p, reproduction_level = pmin(rl_in, RECAP_VEC))), silent = TRUE)
...
if (mode == "run") {
  ...
  if (length(todo)) {
    ...
    clusterExport(cl, c("BASE", "DR", "SPN", "GAMMA0", "AGE_TGT", "K", "KEEP",
                        ...
                        "RECAP", "RECAP_VEC", "STEADY_TMAX", "RAMP_TMAX",
                        "SPINUP_YEARS",
