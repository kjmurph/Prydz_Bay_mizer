...
BIOMASS_METRICS <- c("total_biomass", "whale_biomass", "baleen_biomass",
                     "sperm_biomass", "apex_biomass", "minke_biomass",
                     "seal_biomass", "fish_biomass",
                     "krill_biomass", "ltl_biomass")
...
STRUCTURAL_METRICS_PLOT <- c(
  "spectrum_slope", "spectrum_intercept", "spectrum_mle_exponent",
  "spectrum_lcd_exponent", "spectrum_lbnbiom_slope",
...
# Set to Antarctic krill w_min from trait_groups_params_vCWC_v4.csv.
SPECTRUM_MIN_W <- 1.104292e-06
...
METRIC_DISPLAY <- list(
  ...
  krill_biomass            = list(name = "Krill Biomass",              group = "Biomass",    order = 9),
  ...
  spectrum_lbnbiom_slope   = list(name = "NBS Slope (LBNbiom)",        group = "Structural", order = 13.2),
  ...
  production_biomass_ratio = list(name = "Community P/B (Krill+)",     group = "Structural", order = 20),
...
plot_combined_heatmap <- function(biomass_summary, structural_summary,
  ...
  p_bio <- ggplot(bio_data, aes(x = decade, y = display_name, fill = prop_value)) +
    ...
    scale_fill_gradientn(
      ...
      limits = c(0, 1), na.value = "grey80",
      name = "Proportion below\nthreshold", labels = scales::percent) +
    labs(x = "", y = "", tag = "Biomass Indicators") +
    theme_minimal(base_size = 14.6) +
    theme(
      axis.text.x = element_blank(),
      axis.ticks.x = element_blank(),
      axis.text.y = element_text(size = 13.2),
      plot.tag = element_text(size = 15.9, face = "bold", angle = 90),
  ...
  p_str <- ggplot(str_data, aes(x = decade, y = display_name, fill = conf_level)) +
    ...
    scale_fill_manual(
      ...
      name = "Confidence\nin change") +
    labs(x = "Decade", y = "", tag = "Structural Indicators") +
    theme_minimal(base_size = 14.6) +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, size = 13.2),
      axis.text.y = element_text(size = 13.2),
      plot.tag = element_text(size = 15.9, face = "bold", angle = 90),
      plot.tag.position = "left",
      legend.position = "none",
