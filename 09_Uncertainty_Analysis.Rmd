...
run_enhanced_combined_uncertainty_sims <- function(params,
                                                   ...
                                                   abundance_sd = 1,
                                                   gamma_sd = 1,
                                                   all_species_abundance = TRUE,      # NEW: Default to varying all species
                                                   species_to_vary = NULL,            # NEW: Named vector of species to vary if all_species_abundance = FALSE
                                                   effort_scen,
                                                   enhanced_steady_tol = 0.002,
                                                   enhanced_steady_t_max = 2000,     # INCREASED: More time for extreme parameter variations to converge
                                                   spinup_years = 118,
                                                   t_start = 1841,
                                                   sim_years = 170,
...
run_single_enhanced_sim <- function(i, params, catchability_sd, abundance_sd, gamma_sd,
  ...
  result <- tryCatch({
    ...
    if (nrow(biomass_matrix) < check_years) {
      return(list(success = FALSE, error = "insufficient data for stability check"))
    }
    ...
    if (!all_stable) {
      unstable_species <- names(cv_by_species)[cv_by_species >= stability_threshold]
      return(list(success = FALSE, error = paste("stability test failed:", paste(unstable_species, collapse = ", "))))
    }
...
# Robust steady-state function for extreme parameter variations
robust_steady_state <- function(params, tol = 0.005, t_max = 2000, preserve_erepro = TRUE, 
                               max_erepro = 10, verbose = FALSE, sim_id = NULL) {
  
  # Try progressively relaxed tolerance levels if initial attempt fails
  tolerance_levels <- c(tol, tol * 2, tol * 5, tol * 10)
  time_limits <- c(t_max, min(t_max * 1.5, 3000), min(t_max * 2, 4000), min(t_max * 3, 5000))
  
  for (attempt in 1:length(tolerance_levels)) {
    current_tol <- tolerance_levels[attempt]
    current_t_max <- time_limits[attempt]
    
    if (verbose && !is.null(sim_id) && attempt > 1) {
      cat("    Sim", sim_id, ": Steady state attempt", attempt, "with tolerance", current_tol, 
          "and t_max", current_t_max, "\n")
    }
    
    result <- tryCatch({
      if (preserve_erepro) {
        steady_params <- steady(params, tol = current_tol, 
                               t_max = current_t_max, preserve = c("erepro"))
      } else {
        steady_params <- steady(params, tol = current_tol, 
                               t_max = current_t_max)
      }
      ...
      if (any(is.na(test_sp_params$gamma)) || any(is.infinite(test_sp_params$gamma)) ||
          any(test_sp_params$gamma <= 0) || any(test_sp_params$gamma > 1000)) {
        return(list(success = FALSE, error = "Invalid gamma values after steady state"))
      }
      
      # Additional validation for other critical parameters
      if (any(is.na(test_sp_params$h)) || any(is.infinite(test_sp_params$h)) ||
          any(test_sp_params$h <= 0)) {
        return(list(success = FALSE, error = "Invalid h values after steady state"))
      }
      
      # Check erepro constraints if not preserving
      if (!preserve_erepro) {
        current_erepro <- test_sp_params$erepro
        if (any(is.na(current_erepro)) || any(is.infinite(current_erepro))) {
          return(list(success = FALSE, error = "Invalid erepro values after steady state"))
        }
        if (any(current_erepro > max_erepro, na.rm = TRUE)) {
          return(list(success = FALSE, error = "erepro constraint violated"))
        }
      ...
      return(list(success = TRUE, params = steady_params, tolerance_used = current_tol, 
                 time_used = current_t_max, attempts = attempt))
      
    }, warning = function(w) {
      if (grepl("did not converge", w$message, ignore.case = TRUE)) {
        return(list(success = FALSE, error = paste("Convergence failed at tolerance", current_tol, ":", w$message)))
      } else {
        # Continue with warning but try to validate result
        if (preserve_erepro) {
          steady_params <- steady(params, tol = current_tol, 
                                 t_max = current_t_max, preserve = c("erepro"))
        } else {
        ...
        test_sp_params <- species_params(steady_params)
        if (any(is.na(test_sp_params$gamma)) || any(is.infinite(test_sp_params$gamma))) {
          return(list(success = FALSE, error = "Warning led to invalid parameters"))
        }
        
        return(list(success = TRUE, params = steady_params, tolerance_used = current_tol, 
                   time_used = current_t_max, attempts = attempt, warning = w$message))
      }
    }, error = function(e) {
      return(list(success = FALSE, error = paste("Error at tolerance", current_tol, ":", e$message)))
    })
    
    # If successful, return immediately
    if (result$success) {
      if (verbose && !is.null(sim_id)) {
        cat("    Sim", sim_id, ": Steady state achieved after", result$attempts, 
            "attempts (tolerance:", result$tolerance_used, ")\n")
      }
      return(result)
    }
    
    # If this was the last attempt, log the failure
    if (attempt == length(tolerance_levels) && verbose && !is.null(sim_id)) {
      cat("    Sim", sim_id, ": All steady state attempts failed. Last error:", result$error, "\n")
    }
  }
...
# ggsave("yield_climate_fishing_timeseries_baseline.png", p_biomass_climate_fishing_baseline, width = 12, height = 8, dpi = 300)
```
...
# Run enhanced uncertainty analysis with parallel processing
cat("Running Monte Carlo uncertainty analysis with parallel processing...\n")
...
use_parallel <- TRUE  # Set to FALSE if you want to use sequential processing

if (use_parallel && cores_to_use > 1) {
  cat("Running in PARALLEL mode...\n")
  
  monte_carlo_results <- run_enhanced_combined_uncertainty_sims_parallel(
    params = params_1841_2010_climate_only,
    n_sims = 50,  # Small number for testing - increase for production runs
    catchability_sd = 5,   # SD = 5: Explore very wide fishing efficiency range (exp(±5) = 0.007 to 148x)
    abundance_sd = 5,      # SD = 5: Extreme abundance variation to capture unknown recovery dynamics  
    gamma_sd = 5,          # SD = 5: Wide foraging efficiency exploration for recovery scenarios
    all_species_abundance = TRUE,  # NEW: Vary all species by default for historical recovery problem
    # species_to_vary = c("minke whales", "orca", "sperm whales", "baleen whales"),  # Alternative: specify particular species
    effort_scen = combined_effort_array,
    enhanced_steady_tol = 0.005,
    enhanced_steady_t_max = 2000,  # INCREASED: Allow more time for extreme parameter combinations to converge
    spinup_years = 118,
    t_start = 1841,
    sim_years = 170,
    preserve_erepro = TRUE,
