...
if (exists("enhanced_sims") && length(enhanced_sims) > 0) {
  ...
  # Enhanced faceted yield plot function (matching original code structure)
  create_faceted_yield_plot <- function(sim_list, observed_data = NULL, baseline_sim = NULL) {
    
    # Function to extract and trim yield data (matching trimYield function)
    trimYield <- function(sim) {
      tryCatch({
    ...
    cat("Extracting yield data from", length(sim_list), "simulations using trimYield approach...\n")
    
    # Extract yield data from all LHS simulations (matching your approach)
    sim_catches <- lapply(sim_list, trimYield)
    
    # Add simulation IDs
    for (i in 1:length(sim_catches)) {
      if (!is.null(sim_catches[[i]])) {
        sim_catches[[i]]$sim <- i
      }
    }
    
    # Remove NULL results and combine
    sim_catches <- sim_catches[!sapply(sim_catches, is.null)]
    if (length(sim_catches) == 0) {
    ...
    # Calculate summary statistics (matching your exact approach)
    sim_mean_c <- df_sim_catch %>% 
      group_by(Species, Year) %>% 
      summarise(Yield = median(Yield), .groups = 'drop')
    
    sim_lower_c <- df_sim_catch %>% 
      group_by(Species, Year) %>% 
      summarise(Yield = min(Yield), .groups = 'drop') 
    
    sim_upper_c <- df_sim_catch %>% 
      group_by(Species, Year) %>% 
      summarise(Yield = max(Yield), .groups = 'drop') 
    
    # Create combined data frame (matching your sim1_c structure)
    sim1_c <- data.frame(
    ...
    if (!is.null(baseline_sim)) {
      tryCatch({
        baseline_yield <- plotYield(baseline_sim, return_data = TRUE)
        # Convert to long format if needed
        if (is.matrix(baseline_yield) || is.data.frame(baseline_yield)) {
          baseline_yield <- baseline_yield %>%
            as.data.frame() %>%
            mutate(Year = as.numeric(rownames(.))) %>%
            pivot_longer(cols = -Year, names_to = "Species", values_to = "Baseline_Yield")
        }
      }, error = function(e) {
    ...
    fished_species <- unique(sim1_c$Species)
    cat("Fished species found:", paste(fished_species, collapse = ", "), "\n")
    
    # Ensure species factor ordering
    sim1_c$Species <- factor(sim1_c$Species, levels = intersect(species_order, fished_species))
    if (!is.null(baseline_yield)) {
      baseline_yield$Species <- factor(baseline_yield$Species, levels = intersect(species_order, fished_species))
    }
    
    # Create the plot (matching your exact ggplot structure)
    p <- ggplot(sim1_c, aes(x = Year, y = sim_mean, colour = Species)) +
      facet_wrap(~Species, scales = "free_y", 
                 labeller = labeller(Species = species.labs)) +
      # Add uncertainty ribbon (matching your approach)
      geom_ribbon(aes(ymin = sim_lower, ymax = sim_upper, fill = Species), 
                  alpha = 0.3, colour = NA) +
      # Add LHS mean line (colored by species)
      geom_line(aes(y = sim_mean, colour = Species), size = 1) +
      # Add vertical reference lines
      geom_vline(xintercept = 1961, linetype = "dashed", alpha = 0.6) +
      geom_vline(xintercept = 2010, linetype = "dashed", alpha = 0.6) +
      scale_fill_viridis_d(name = "Species") +
      scale_color_viridis_d(name = "Species") +
      labs(
    ...
    if (!is.null(baseline_yield)) {
      p <- p + 
        geom_line(data = baseline_yield,
                  aes(x = Year, y = Baseline_Yield),
                  color = "grey50", linetype = "dashed", size = 0.8,
                  inherit.aes = FALSE)
    }
    
    # Add observed data if provided (matching your approach)
    if (!is.null(observed_data)) {
  ...
  # Enhanced faceted biomass plot function
  create_faceted_biomass_plot <- function(sim_list, obs_biomass_data = NULL, baseline_sim = NULL) {
    ...
    for(i in 1:length(sim_list)) {
      tryCatch({
        ...
        sim_biomass <- getBiomass(sim_list[[i]])
        
        # Convert to data frame
        if (is.matrix(sim_biomass) || is.data.frame(sim_biomass)) {
          sim_df <- sim_biomass %>% 
            as.data.frame() %>%
            mutate(Year = as.numeric(rownames(.))) %>%
    ...
    if (nrow(sim_biomass_range) == 0) {
      cat("No valid biomass data extracted\n")
      return(NULL)
    }
    
    # Calculate summary statistics for each species and year
    sim_summary <- sim_biomass_range %>%
      group_by(Year, Species) %>%
      summarize(
        Min = min(Biomass, na.rm = TRUE),
        Max = max(Biomass, na.rm = TRUE),
    ...
    baseline_biomass <- NULL
    if (!is.null(baseline_sim)) {
      tryCatch({
        baseline_biomass <- getBiomass(baseline_sim)
        # Convert to long format
        if (is.matrix(baseline_biomass) || is.data.frame(baseline_biomass)) {
          baseline_biomass <- baseline_biomass %>%
            as.data.frame() %>%
            mutate(Year = as.numeric(rownames(.))) %>%
            pivot_longer(cols = -Year, names_to = "Species", values_to = "Baseline_Biomass")
          
          baseline_biomass$Species <- factor(baseline_biomass$Species, levels = species_order)
        }
      }, error = function(e) {
    ...
    p <- ggplot() +
      # Add ribbon showing full range from all simulations (light)
      geom_ribbon(data = sim_summary,
                  aes(x = Year, ymin = Min, ymax = Max, fill = Species),
                  alpha = 0.3) +
      # Add ribbon showing interquartile range (darker)
      geom_ribbon(data = sim_summary, 
                  aes(x = Year, ymin = Q25, ymax = Q75, fill = Species), 
                  alpha = 0.5) +
      # Plot mean line from simulations (solid)
    ...
    # Add baseline model line (grey dashed) if available
    if (!is.null(baseline_biomass)) {
      p <- p + 
        geom_line(data = baseline_biomass,
                  aes(x = Year, y = Baseline_Biomass),
                  color = "grey50", linetype = "dashed", size = 0.8,
                  inherit.aes = FALSE)
    }
    ...
    if (!is.null(obs_biomass_data)) {
      ...
      common_species <- intersect(unique(sim_summary$Species), unique(obs_biomass_data$Species))
      obs_filtered <- obs_biomass_data %>% filter(Species %in% common_species)
      
      if (nrow(obs_filtered) > 0) {
        # Add observed uncertainty ribbon (±10%, light grey, only in relevant facets)
        p <- p +
