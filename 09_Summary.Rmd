...
calculate_sensitivity_indices <- function(results, lhs_params, 
  ...
  for (i in seq_along(valid_results)) {
    ...
    if (output_metric == "total_biomass") {
      output_values[i] <- sum(result$final_state$biomass, na.rm = TRUE)
    } else if (output_metric == "total_yield") {
      output_values[i] <- sum(result$yield_comparison$simulated_yield, na.rm = TRUE)
    } else if (output_metric == "yield_rmse") {
      output_values[i] <- result$yield_comparison$yield_rmse
    } else if (output_metric == "biomass_stability") {
      output_values[i] <- result$stability_metrics$cv_biomass
    }
  }
...
calculate_species_sensitivity <- function(results, lhs_params, marine_mammal_species) {
  ...
  all_species <- rownames(first_result$final_state$biomass)
  ...
  for (species in all_species) {
    ...
    for (i in seq_along(valid_results)) {
      ...
      # Get species biomass
      species_biomass[i] <- result$final_state$biomass[species, ncol(result$final_state$biomass)]
