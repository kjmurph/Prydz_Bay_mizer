...
This section implements an improved adaptive approach that checks RMSE every 50 simulations and refines the parameter search space based on performance, with special attention to species with poor fits.

### Load Adaptive Functions

```{r load-adaptive-functions}
# Source the new adaptive uncertainty analysis functions
source("adaptive_uncertainty_functions.R")
```

### Run Adaptive Uncertainty Analysis

The adaptive approach gradually refines the search space, focusing on parameter combinations that show promise while specifically tracking species that have poor fits (like Antarctic krill and minke whales).
...
# Run adaptive uncertainty analysis
# This will check RMSE every 50 simulations and refine the search space
adaptive_results <- run_adaptive_uncertainty_analysis(
  params = params_1841_2010_fishing_only,
  n_total_sims = 500,  # Total number of simulations
  check_interval = 50,  # Check and refine every 50 simulations
