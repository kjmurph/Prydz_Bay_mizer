...
calculate_sensitivity_indices <- function(results, lhs_params, 
  ...
  for (i in seq_along(valid_results)) {
    ...
    } else if (output_metric == "total_yield") {
      # Get total yield from performance metrics
      output_values[i] <- sum(result$performance$simulated_yield, na.rm = TRUE)
    } else if (output_metric == "yield_rmse") {
      output_values[i] <- result$performance$yield_rmse
  ...
  # Calculate correlation-based sensitivity (Pearson and Spearman)
  sensitivity_pearson <- cor(param_matrix, output_values, method = "pearson")
  sensitivity_spearman <- cor(param_matrix, output_values, method = "spearman")
  
  # Calculate variance-based sensitivity (simple approach)
  # Normalize parameters to [0,1] for comparison
  param_norm <- apply(param_matrix, 2, function(x) (x - min(x)) / (max(x) - min(x)))
  
  # Calculate partial variance contributions
  total_var <- var(output_values)
  partial_vars <- numeric(3)
  names(partial_vars) <- colnames(param_matrix)
...
calculate_species_sensitivity <- function(results, lhs_params, marine_mammal_species) {
  ...
  all_species <- rownames(sim_biomass)
...
run_comprehensive_sensitivity_analysis <- function(results, lhs_params, marine_mammal_species) {
  ...
  # Overall sensitivity for different metrics
  metrics <- c("total_biomass", "total_yield", "yield_rmse", "biomass_stability")
