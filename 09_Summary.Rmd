...
if (exists("enhanced_uncertainty_results") && !is.null(enhanced_uncertainty_results$simulations)) {
  ...
  if (length(test_sims) > 0) {
    ...
    for (i in seq_along(test_sims)) {
      ...
      # Test biomass extraction using new method
      if (!is.null(result$simulation)) {
        sim_biomass <- getBiomass(result$simulation)
        total_biomass <- sum(sim_biomass[nrow(sim_biomass), ], na.rm = TRUE)
        
        cat(sprintf("  Sim %d: Total biomass = %.2e (extracted successfully)\n", 
                   i, total_biomass))
        ...
        cat(sprintf("  Sim %d: ERROR - No simulation object found\n", i))
      }
    }
    
    cat("✓ Data extraction validation completed successfully!\n")
    cat("✓ getBiomass() function works correctly with LHS simulation objects\n")
    cat("✓ Sensitivity analysis should now run without '$' operator errors\n")
    
    # Additional validation: Test that full result structure is correct for sensitivity analysis
    if (exists("enhanced_uncertainty_results") && !is.null(enhanced_uncertainty_results$simulations)) {
      # Check if we can simulate the expected result structure for sensitivity analysis
      test_result <- list(
        simulation = enhanced_uncertainty_results$simulations[[1]],
        performance = list(simulated_yield = 100, yield_rmse = 0.1),
        params_info = list(sim_id = 1)
      )
      
      # Test the corrected data extraction method
      tryCatch({
        sim_biomass <- getBiomass(test_result$simulation)
        total_biomass <- sum(sim_biomass[nrow(sim_biomass), ], na.rm = TRUE)
        cat(sprintf("✓ Full result structure validation: Total biomass = %.2e\n", total_biomass))
        cat("✓ Sensitivity analysis data structure is now correctly formatted\n")
      }, error = function(e) {
        cat("⚠ Validation warning:", conditionMessage(e), "\n")
      })
    }
