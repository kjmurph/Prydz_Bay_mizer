...
# Create new yield plot in the style you requested
p_yield_styled <- ggplot() +
  # Add observed data points first (black points)
  geom_point(data = yield_ts_tidy, 
             aes(x = Year, y = Yield), 
             colour = "black", size = 0.5) +
  # Add uncertainty ribbon (grey)
  geom_ribbon(data = uncertainty_yield_data, 
              aes(x = Year, ymin = sim_lower, ymax = sim_upper), 
              alpha = 0.3, colour = NA, fill = "grey") +
  # Add ensemble mean line (species colors)
  geom_line(data = uncertainty_yield_data, 
            aes(x = Year, y = sim_mean, colour = Species), 
            linewidth = 1) +
  # Add baseline model line (grey long dashed)
  geom_line(data = baseline_yield_data, 
            aes(x = Year, y = Yield), 
            color = "grey60", linetype = "longdash", linewidth = 1.2) +
  # Add vertical reference lines
  geom_vline(xintercept = 1961, linetype = "dashed", alpha = 0.7) + 
  geom_vline(xintercept = 2010, linetype = "dashed", alpha = 0.7) +
  ...
  labs(
    title = "Enhanced Optimal Simulation: Yield with Uncertainty (Styled)",
    subtitle = "Grey ribbon = 95% CI, Colored lines = ensemble mean, Grey dashed = baseline model",
    y = "Yield"
  )

print(p_yield_styled)
ggsave("plots/enhanced_optimal_simulation_yield_styled.png", p_yield_styled,
       width = 16, height = 12, dpi = 300)
