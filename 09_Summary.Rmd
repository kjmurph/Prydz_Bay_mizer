...
if (exists("enhanced_sims") && length(enhanced_sims) > 0) {
  ...
  # Enhanced faceted yield plot function (only fished species)
  create_faceted_yield_plot <- function(sim_list, observed_data = NULL) {
    
    # Extract yield data from simulations using a trimming function
    cat("Extracting yield data from", length(sim_list), "simulations...\n")
    
    # Function to extract and trim yield data (similar to trimYield)
    extract_yield <- function(sim) {
      tryCatch({
        yield_data <- getYield(sim)
        
        # Convert to data frame
        if (is.matrix(yield_data) || is.data.frame(yield_data)) {
          yield_df <- as.data.frame(yield_data)
          yield_df$Year <- as.numeric(rownames(yield_df))
          
          # Reshape to long format
          yield_long <- yield_df %>%
            pivot_longer(cols = -Year, names_to = "Species", values_to = "Yield") %>%
            filter(Yield > 0)  # Only include species with actual yield
    ...
    sim_yields <- lapply(1:length(sim_list), function(i) {
      yield_data <- extract_yield(sim_list[[i]])
    ...
    df_sim_yield <- do.call(rbind, sim_yields)
    
    # Calculate summary statistics
    sim_summary <- df_sim_yield %>%
    ...
    # Filter to only include fished species (those present in yield data)
    fished_species <- unique(sim_summary$Species)
    cat("Fished species found:", paste(fished_species, collapse = ", "), "\n")
    
    # Ensure species factor ordering (only for fished species)
    sim_summary$Species <- factor(sim_summary$Species, levels = intersect(species_order, fished_species))
    
    # Create the plot
    p <- ggplot(sim_summary, aes(x = Year, y = sim_mean, colour = Species)) +
      facet_wrap(~Species, scales = "free_y", 
                 labeller = labeller(Species = species.labs)) +
      ...
      geom_ribbon(aes(ymin = sim_lower, ymax = sim_upper, fill = Species), 
                  alpha = 0.3, colour = NA) +
      # Add mean line
      geom_line(aes(y = sim_mean), size = 1) +
