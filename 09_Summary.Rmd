...
# Plot yield comparison using same data as the first plots for consistency
p_yield_optimal_v2 <- ggplot() +
  # Add uncertainty ribbon first (behind other elements)
  geom_ribbon(data = uncertainty_yield_data, 
              aes(x = Year, ymin = sim_lower, ymax = sim_upper), 
              alpha = 0.3, fill = "lightblue") +
  # Add median line from uncertainty analysis (should be close to optimal)
  ...
  geom_line(data = baseline_yield_data, 
            aes(x = Year, y = Yield), 
            color = "gray60", linetype = "dashed", linewidth = 1) +
  # Add optimal model line (from composite results)
  geom_line(data = plotYieldGear(optimal_results_composite$optimal_sim, return_data = TRUE),
            aes(x = Year, y = Yield, color = Species), linewidth = 1.2) +
  # Add observed data points
  geom_point(data = yield_ts_tidy, 
  ...
  geom_point(data = yield_ts_tidy, 
             ...
             shape = 1, size = 1, colour = "black") +
  # Add vertical reference lines
  geom_vline(xintercept = 1961, linetype = "dotted", alpha = 0.7) + 
  geom_vline(xintercept = 2010, linetype = "dotted", alpha = 0.7) +
  ...
  labs(
    title = "Enhanced Optimal Simulation: Yield with Uncertainty",
    subtitle = "Gray dashed = baseline model, Colored solid = optimal fit, Blue dotted = ensemble median, Blue ribbon = 95% CI",
    x = "Year",
    y = "Yield"
  )

print(p_yield_optimal_v2)
ggsave("plots/enhanced_optimal_simulation_yield_v2.png", p_yield_optimal_v2,
       width = 14, height = 10, dpi = 300)

# Plot biomass comparison using same approach
p_biomass_optimal_v2 <- ggplot() +
  # Add uncertainty ribbon first (behind other elements)
  geom_ribbon(data = uncertainty_biomass_data, 
              aes(x = Year, ymin = sim_lower, ymax = sim_upper), 
  ...
  labs(
    title = "Enhanced Optimal Simulation: Biomass with Uncertainty",
    subtitle = "Gray dashed = baseline model, Colored solid = optimal fit, Green dotted = ensemble median, Green ribbon = 95% CI",
    x = "Year",
    y = "Biomass"
  )

print(p_biomass_optimal_v2)
ggsave("plots/enhanced_optimal_simulation_biomass_v2.png", p_biomass_optimal_v2,
       width = 14, height = 10, dpi = 300)
```
