...
if (exists("enhanced_sims") && length(enhanced_sims) > 0) {
  ...
  create_faceted_biomass_plot <- function(sim_list, obs_biomass_data = NULL) {
    ...
    p <- ggplot() +
      ...
      geom_line(data = sim_summary,
                ...
                linewidth = 0.8) +
      # Add baseline model line if available (dashed) - you might need to provide this
    ...
    if (!is.null(obs_biomass_data)) {
      ...
      if (nrow(obs_filtered) > 0) {
        # Add observed uncertainty ribbon (light grey, only in relevant facets)
        p <- p +
