...
trimYield<-function(sim){
  ...
  y$Yield <- y$Yield / 1e6  # Convert grams to tonnes
  return(y)
...
calculate_enhanced_performance_metrics <- function(sim_object,
  ...
  # Get modeled yield and convert from grams to tonnes
  mod_yield <- getYield(sim_object)
  df_mod <- reshape2::melt(mod_yield)
  names(df_mod) <- c("Year", "Species", "Yield")
  df_mod$Yield <- df_mod$Yield / 1e6  # Convert grams to tonnes
  ...
  # Convert observed yield to tonnes (assuming it's in grams to match model output)
  yield_obs_data$Yield <- yield_obs_data$Yield / 1e6  # Convert grams to tonnes
...
run_enhanced_combined_uncertainty_sims <- function(params,
  ...
  if (length(performance_results) > 0) {
    ...
    cat("  Weighted RMSE - Mean:", round(mean(rmse_scores), 4), "tonnes",
        "Range:", round(min(rmse_scores), 4), "-", round(max(rmse_scores), 4), "tonnes\n")
  }
...
find_optimal_parameters_enhanced <- function(sim_results_list, metric = "composite_score") {
  ...
  cat("Weighted RMSE:", round(optimal_performance$total_weighted_rmse, 4), "tonnes\n")
  cat("Weighted MAE:", round(optimal_performance$total_weighted_mae, 4), "tonnes\n")
  cat("Weighted Bias:", round(optimal_performance$total_weighted_bias, 4), "tonnes\n")
...
if (exists("enhanced_uncertainty_results") && length(enhanced_uncertainty_results$performance) > 0) {
  ...
  if (!"composite_score_weighted" %in% available_metrics) {
    ...
    cat("Converting gram-scale metrics to tonne-scale for composite scores...\n")
    ...
    for (i in 1:length(enhanced_uncertainty_results$performance)) {
      ...
      # Convert metrics from grams to tonnes for composite score calculation
      rmse_tonnes <- perf$total_weighted_rmse / 1e6
      mae_tonnes <- perf$total_weighted_mae / 1e6
      bias_tonnes <- perf$total_weighted_bias / 1e6
      correlation <- perf$avg_correlation
      
      # Calculate weighted composite (40-30-20-10) using tonne-scale values
      perf$composite_score_weighted <- (rmse_tonnes * 0.4) +
                                       (mae_tonnes * 0.3) +
                                       (abs(bias_tonnes) * 0.2) +
                                       ((1 - correlation) * 0.1)
      
      # Calculate equal composite (25-25-25-25) using tonne-scale values
      perf$composite_score_equal <- (rmse_tonnes * 0.25) +
                                    (mae_tonnes * 0.25) +
                                    (abs(bias_tonnes) * 0.25) +
                                    ((1 - correlation) * 0.25)
      
      # Also update the original metrics to tonne-scale for consistency
      perf$total_weighted_rmse_tonnes <- rmse_tonnes
      perf$total_weighted_mae_tonnes <- mae_tonnes
      perf$total_weighted_bias_tonnes <- bias_tonnes
      
      # Update the performance record
      enhanced_uncertainty_results$performance[[i]] <- perf
    }
    cat("Successfully added tonne-scale composite score variants!\n")
  }
...
if (exists("optimal_results_composite_weighted") && exists("optimal_results_composite_equal")) {
  ...
  cat("  RMSE:", round(weighted_perf$total_weighted_rmse, 4), "tonnes\n")
  cat("  MAE:", round(weighted_perf$total_weighted_mae, 4), "tonnes\n") 
  cat("  Bias:", round(abs(weighted_perf$total_weighted_bias), 4), "tonnes\n")
  ...
  cat("  RMSE:", round(equal_perf$total_weighted_rmse, 4), "tonnes\n")
  cat("  MAE:", round(equal_perf$total_weighted_mae, 4), "tonnes\n")
  cat("  Bias:", round(abs(equal_perf$total_weighted_bias), 4), "tonnes\n")
  ...
  cat("  RMSE Δ:", round(equal_perf$total_weighted_rmse - weighted_perf$total_weighted_rmse, 4), "tonnes\n")
  cat("  MAE Δ:", round(equal_perf$total_weighted_mae - weighted_perf$total_weighted_mae, 4), "tonnes\n")
  cat("  Bias Δ:", round(abs(equal_perf$total_weighted_bias) - abs(weighted_perf$total_weighted_bias), 4), "tonnes\n")
  ...
  cat("Weighted RMSE:", round(best_model_performance$total_weighted_rmse, 4), "tonnes\n")
  cat("Weighted MAE:", round(best_model_performance$total_weighted_mae, 4), "tonnes\n")
  cat("Weighted Bias:", round(best_model_performance$total_weighted_bias, 4), "tonnes\n")
...
p_yield_optimal <- plotYield(optimal_results_composite$optimal_sim) + 
  ...
  # Add optimal model line (solid black for best fit) - use ensemble version for consistency (data already converted to tonnes)
  ...
  geom_point(data = yield_ts_tidy, 
             ...
             size = 1) +
  geom_point(data = transform(yield_ts_tidy, Yield = Yield / 1e6), 
             aes(x = Year, y = Yield), 
...
p_yield_adaptive <- plotYield(optimal_sim_adaptive) +
  facet_wrap(~Species, scales = "free_y") +
  # Convert plotYield output from grams to tonnes
  scale_y_continuous(labels = function(x) x / 1e6) +
  # Add observed data points (convert from grams to tonnes)
  geom_point(data = transform(yield_ts_tidy, Yield = Yield / 1e6),
             aes(x = Year, y = Yield, colour = Species),
             size = 1) +
  geom_point(data = transform(yield_ts_tidy, Yield = Yield / 1e6),
  ...
  labs(title = "Optimal Simulation from Adaptive Search",
       subtitle = paste("RMSE =", round(adaptive_results$optimal$optimal_rmse, 3), "tonnes"),
       y = "Yield (tonnes)")
...
for (sp in species_focus) {
  # Get modeled yield for this species and convert to tonnes
  ...
  # Get observed yield for this species and convert to tonnes
  obs_sp <- yield_ts_tidy[yield_ts_tidy$Species == sp, ]
  obs_sp$Yield <- obs_sp$Yield / 1e6  # Convert grams to tonnes
