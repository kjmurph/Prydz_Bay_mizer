...
# This function randomizes both catchability for fished species and initial abundance for marine mammals
run_combined_uncertainty_sims <- function(params,
  ...
  for(i in 1:n_sims) {
    ...
    # Store parameter combination for later analysis
...
# Plot yield comparison for optimal simulation
p_yield_optimal <- plot_yield_comparison(
  sim_object = optimal_results$optimal_sim,
  yield_obs_data = yield_ts_tidy,
  free_y_scale = TRUE,
  plot_title = "Optimal Simulation: Modeled vs. Observed Yield"
)
print(p_yield_optimal)
ggsave("plots/optimal_simulation_yield.png", p_yield_optimal,
       width = 14, height = 10, dpi = 300)

# Plot biomass comparison for optimal simulation
p_biomass_optimal <- plot_biomass_comparison(
  sim_object = optimal_results$optimal_sim,
  plot_title = "Optimal Simulation: Modeled vs. Observed Biomass"
)
...
This combined uncertainty assessment explores both catchability uncertainty for fished species and abundance scaling uncertainty for marine mammals. The analysis:

1. **Parameter Space Exploration**: Systematically varies both catchability and marine mammal abundance to explore the parameter space
2. **Optimal Parameter Identification**: Finds the parameter combination that minimizes RMSE between modeled and observed yields
3. **Uncertainty Quantification**: Provides distributions of model performance across the parameter uncertainty range
4. **Species-Specific Analysis**: Identifies which species contribute most to model-observation discrepancies
5. **Sensitivity Analysis**: Reveals correlations between parameters and model performance

The optimal parameters identified through this analysis can be used for more accurate historical reconstructions and future projections.
