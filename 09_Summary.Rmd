...
if (validation_test$success_rate >= 80) {
  ...
  success_rate <- mean(sapply(enhanced_uncertainty_results, function(x) x$success)) * 100
  cat("\n=== ENHANCED UNCERTAINTY ANALYSIS RESULTS ===\n")
  cat("Success rate:", round(success_rate, 1), "%\n")
  
  if (success_rate >= 70) {
    successful_sims <- enhanced_uncertainty_results[sapply(enhanced_uncertainty_results, function(x) x$success)]
    ...
    enhanced_simulations <- lapply(successful_sims, function(x) x$simulation)
    saveRDS(enhanced_simulations, "enhanced_simulations_for_plotting.RDS")
...
This section implements advanced adaptive optimization that learns from simulation results:
