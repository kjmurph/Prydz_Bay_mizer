...
if (exists("enhanced_uncertainty_results") && length(enhanced_uncertainty_results$performance) > 0) {
  ...
  if (!"composite_score_weighted" %in% available_metrics) {
    ...
    for (i in 1:length(enhanced_uncertainty_results$performance)) {
      ...
      # Calculate weighted composite (40-30-20-10)
      perf$composite_score_weighted <- (perf$total_weighted_rmse * 0.4) +
                                       (perf$total_weighted_mae * 0.3) +
                                       (abs(perf$total_weighted_bias) * 0.2) +
                                       ((1 - perf$avg_correlation) * 0.1)
      
      # Calculate equal composite (25-25-25-25)
      perf$composite_score_equal <- (perf$total_weighted_rmse * 0.25) +
                                    (perf$total_weighted_mae * 0.25) +
                                    (abs(perf$total_weighted_bias) * 0.25) +
                                    ((1 - perf$avg_correlation) * 0.25)
