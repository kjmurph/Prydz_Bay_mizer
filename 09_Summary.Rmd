...
combined_uncertainty_results <- run_combined_uncertainty_sims(
  ...
  n_sims = 1000,  # Number of simulations (can be thousands with parallel processing)
  catchability_sd = .5,  # 20% CV for catchability
  abundance_sd = 2,  # 30% CV for marine mammal abundance
  marine_mammal_species = c("minke whales", "orca", "sperm whales", "baleen whales"),
  ...
  t_start = 1841,
  sim_years = 170,
  preserve_erepro = F,  # Set to FALSE to filter out simulations with erepro > 1
...
# Save results
saveRDS(combined_uncertainty_results, "combined_uncertainty_results_flex_erepro_catchSD_0.5_abSD_2.RDS")
```

## Analyze Results and Find Optimal Parameters

```{r analyze-combined-results, eval=FALSE}
# First, validate and clean the simulation results to remove any invalid simulations
cat("Validating simulation results...\n")
cleaned_results <- validate_simulation_results(combined_uncertainty_results, verbose = TRUE)

# Find optimal parameters based on RMSE using cleaned results
...
saveRDS(optimal_results, "optimal_parameter_results_flex_erepro_catchSD_0.5_abSD_2.RDS")
