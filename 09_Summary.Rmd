...
if (exists("enhanced_sims") && length(enhanced_sims) > 0) {
  ...
  # Enhanced faceted yield plot function
  create_faceted_yield_plot <- function(sim_list, observed_data = NULL) {
    
    # Calculate yield uncertainty for subset
    cat("Calculating yield uncertainty ribbons...\n")
    yield_uncertainty <- calculate_uncertainty_ribbons(sim_list, getYield)
    
    if (is.null(yield_uncertainty)) {
      cat("Failed to calculate yield uncertainty\n")
      return(NULL)
    }
    
    # Ensure species factor ordering
    if ("species" %in% names(yield_uncertainty)) {
      yield_uncertainty$species <- factor(yield_uncertainty$species, levels = species_order)
    }
    ...
    # Add observed data if provided
    if (!is.null(observed_data)) {
      # Ensure column names and factor ordering
      obs_data_plot <- observed_data
      names(obs_data_plot) <- tolower(names(obs_data_plot))
      
      if ("species" %in% names(obs_data_plot)) {
        obs_data_plot$species <- factor(obs_data_plot$species, levels = species_order)
      }
  ...
  create_faceted_biomass_plot <- function(sim_list, obs_biomass_data = NULL) {
    ...
    # Calculate biomass uncertainty for subset
    cat("Calculating biomass uncertainty ribbons...\n")
    biomass_uncertainty <- calculate_uncertainty_ribbons(sim_list, getBiomass)
    
    if (is.null(biomass_uncertainty)) {
      cat("Failed to calculate biomass uncertainty\n")
      return(NULL)
    }
    
    # Ensure species factor ordering
    if ("species" %in% names(biomass_uncertainty)) {
      biomass_uncertainty$species <- factor(biomass_uncertainty$species, levels = species_order)
    }
    
    # Create base plot
    p <- ggplot() +
      # Model uncertainty ribbons
      ...
      geom_line(data = biomass_uncertainty,
                ...
                size = 1.2) +
      # Add baseline model line if available
      geom_line(data = biomass_uncertainty,
    ...
    # Add observed biomass data if provided
    if (!is.null(obs_biomass_data)) {
      ...
      # Add observed points and uncertainty ribbon
      p <- p +
        geom_ribbon(data = obs_biomass_data,
                    aes(x = Year, ymin = Lower, ymax = Upper),
                    fill = "lightgrey", alpha = 0.3) +
        geom_point(data = obs_biomass_data,
