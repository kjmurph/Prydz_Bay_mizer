...
generate_lhs_parameters <- function(n_sims, 
  ...
  n_params <- n_fished_species + n_marine_mammals
  ...
  cat("  - Marine mammals (abundance):", n_marine_mammals, "\n")
  ...
  # First n_fished_species columns are for catchability (log-normal via normal)
  # Last n_marine_mammals columns are for abundance scaling (log-normal via normal)
  
  # Convert uniform samples to normal samples using inverse normal CDF
  normal_samples <- qnorm(lhs_design)
  
  # Split into catchability and abundance parameter sets
  catchability_samples <- normal_samples[, 1:n_fished_species, drop = FALSE] * catchability_sd
  abundance_samples <- normal_samples[, (n_fished_species + 1):n_params, drop = FALSE] * abundance_sd
  ...
  param_correlations <- cor(cbind(catchability_factors, abundance_factors))
...
run_lhs_enhanced_uncertainty_sims <- function(params,
  ...
  if (parallel && n_cores > 1) {
    ...
    for(i in 1:n_sims) {
      if(i %% 5 == 0) cat("    Processing LHS simulation", i, "of", n_sims, "\n")
