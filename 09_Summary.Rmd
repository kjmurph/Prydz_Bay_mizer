...
enhanced_steady_state <- function(params, 
  ...
  while (attempt <= max_attempts && !steady_successful) {
    ...
    tryCatch({
      ...
      stability_check <- check_biomass_stability_enhanced(test_sim, tolerance = 0.01, 
      ...
      if (is.logical(stability_check) && stability_check) {
        ...
        if (verbose && !is.null(sim_id)) {
          cat("Sim", sim_id, "steady-state achieved but not stable, increasing t_max\n")
        }
        t_max <- t_max * 1.5
      }
    }, error = function(e) {
...
enhanced_spinup <- function(params,
  ...
  is_stable <- check_biomass_stability_enhanced(sim_spinup, 
                                               ...
                                               verbose = FALSE)  # Keep spinup checks quiet
  
  # Handle both return formats
  if (is.list(is_stable)) {
    is_stable <- is_stable$stable
  }
...
run_enhanced_single_sim <- function(sim_id, params, catchability_sd, abundance_sd,
  ...
  if (!sim_result$spinup_stable) {
    return(list(
      ...
      sim_id = sim_id,
      details = "Spinup period did not achieve biomass stability"
    ))
  }
...
## Test Stability Requirements and Find Appropriate Parameters

Before running the full enhanced uncertainty analysis, let's test what stability parameters work with your model:

```{r test-stability-requirements, eval=FALSE}
# Function to test different stability requirements
...
stability_test_results <- test_stability_requirements(
  params = params_1841_2010_climate_only,
  effort_scen = combined_effort_array,
  tolerance_levels = c(0.05, 0.1, 0.15, 0.2, 0.25),  # 5% to 25% CV
  check_years_options = c(20, 30, 50),  # Different time windows
  n_test_sims = 10  # Test with 10 random parameter combinations
)

# Save test results
saveRDS(stability_test_results, "stability_test_results.RDS")
```

## Quick Fix and Test

First, let's define a simple version of the stability test that works with the current functions:

```{r quick-stability-test}
# Simple test function that works with existing functions
...
# Run the quick test
cat("Running quick stability test...\n")
quick_results <- quick_stability_test(params_1841_2010_climate_only, combined_effort_array, n_tests = 3)
```

## Test Corrected Approach - Spinup Stability Only

Let's test the corrected approach where we only check stability during the spinup period:

```{r test-spinup-stability-only}
# Function to test spinup-only stability approach
test_spinup_stability_approach <- function(params, effort_scen, n_tests = 5) {
  ...
  for (i in 1:n_tests) {
    ...
    tryCatch({
      ...
      # Check spinup stability (5% CV over last 50 years of spinup)
      ...
      if (nrow(biomass_spinup) >= 50) {
        ...
        spinup_stable <- all(cv_values < 0.05, na.rm = TRUE)
  ...
  if (success_count > 0) {
    ...
    cat("Spinup CV range:", round(min(spinup_cvs), 1), "-", round(max(spinup_cvs), 1), "% (should be < 5%)\n")
    cat("Fished CV range:", round(min(fished_cvs), 1), "-", round(max(fished_cvs), 1), "% (variability is expected)\n")
    cat("\nThis demonstrates that:\n")
    cat("1. Spinups can achieve stability (< 5% CV)\n")
    cat("2. Fished periods naturally have higher variability (> 5% CV)\n")
...
# Run the test
cat("Testing the corrected approach...\n")
spinup_test_results <- test_spinup_stability_approach(
...
## Run Enhanced Simulations with Corrected Stability Check

Now run the enhanced analysis using the corrected approach:

```{r run-corrected-enhanced-sims}
## Run Enhanced Simulations with Corrected Stability Check

Now run the enhanced analysis using the corrected approach:

```{r run-corrected-enhanced-sims}
# Now that we understand the issue, run enhanced simulations with corrected approach
if (exists("spinup_test_results") && spinup_test_results$success_rate > 0) {
  cat("Spinup-only stability approach validated! Running enhanced simulations...\n\n")
  
  # Use the original enhanced function but with the corrected stability logic
  enhanced_uncertainty_corrected <- run_enhanced_combined_uncertainty_sims(
    params = params_1841_2010_climate_only,
    n_sims = 50,  # Start with 50 to test
    catchability_sd = 0.3,  # 30% CV for catchability
    abundance_sd = 0.5,     # 50% CV for marine mammal abundance
    marine_mammal_species = c("minke whales", "orca", "sperm whales", "baleen whales"),
    effort_scen = combined_effort_array,
    enhanced_steady_tol = 0.005,    # Reasonable tolerance
    enhanced_steady_t_max = 800,    # Reasonable time
    spinup_years = 118,
    spinup_multiplier = 2,          # 2x spinup for good measure
    t_start = 1841,
    sim_years = 170,
    preserve_erepro = TRUE,
    max_erepro = 1,
    ...
    n_cores = NULL,
    parallel = FALSE  # Sequential for better error tracking initially
  )
  
  # Report results
  cat("\n=== CORRECTED ENHANCED SIMULATION RESULTS ===\n")
  cat("Successful simulations:", enhanced_uncertainty_corrected$n_successful, "\n")
  cat("Total attempts:", enhanced_uncertainty_corrected$n_attempts, "\n")
  cat("Success rate:", round(enhanced_uncertainty_corrected$n_successful / enhanced_uncertainty_corrected$n_attempts * 100, 1), "%\n")
  
  if (enhanced_uncertainty_corrected$n_successful > 0) {
    # Calculate performance summary
    performance_metrics <- enhanced_uncertainty_corrected$performance
    
    if (length(performance_metrics) > 0) {
      composite_scores <- sapply(performance_metrics, function(x) x$composite_score)
      weighted_rmse <- sapply(performance_metrics, function(x) x$total_weighted_rmse)
      
      cat("\nPerformance Summary:\n")
      cat("Composite Score - Mean:", round(mean(composite_scores), 4), 
          "Range:", round(min(composite_scores), 4), "-", round(max(composite_scores), 4), "\n")
      cat("Weighted RMSE - Mean:", round(mean(weighted_rmse), 4),
          "Range:", round(min(weighted_rmse), 4), "-", round(max(weighted_rmse), 4), "\n")
      
      # Find optimal simulation
      best_idx <- which.min(composite_scores)
      cat("Best simulation: #", best_idx, "with composite score =", round(composite_scores[best_idx], 4), "\n")
    }
    
    # Save successful results
    saveRDS(enhanced_uncertainty_corrected, "enhanced_uncertainty_results_corrected.RDS")
    
    # If we have good success rate, run larger batch
    success_rate <- enhanced_uncertainty_corrected$n_successful / enhanced_uncertainty_corrected$n_attempts
    if (success_rate > 0.6) {  # If >60% success rate
      cat("\nHigh success rate achieved! Running larger batch of 200 simulations...\n")
      
      enhanced_uncertainty_large <- run_enhanced_combined_uncertainty_sims(
        params = params_1841_2010_climate_only,
        n_sims = 200,
        catchability_sd = 0.4,  # Slightly higher uncertainty
        abundance_sd = 0.8,     # Slightly higher uncertainty
        marine_mammal_species = c("minke whales", "orca", "sperm whales", "baleen whales"),
        effort_scen = combined_effort_array,
        enhanced_steady_tol = 0.002,    # Can use stricter now that we know it works
        enhanced_steady_t_max = 1000,   # Can use longer now
        spinup_years = 118,
        spinup_multiplier = 2,
        t_start = 1841,
        sim_years = 170,
        ...
        yield_obs_data = yield_ts_tidy,
        n_cores = NULL,
        parallel = TRUE  # Can use parallel now
      )
      
      cat("Large batch results:", enhanced_uncertainty_large$n_successful, "successful of", enhanced_uncertainty_large$n_attempts, "\n")
      saveRDS(enhanced_uncertainty_large, "enhanced_uncertainty_results_large_corrected.RDS")
      
      # Use the larger results for final analysis
      enhanced_uncertainty_corrected <- enhanced_uncertainty_large
    }
    
  } else {
    cat("Still getting failures - check spinup stability requirements\n")
  }
  
} else {
  cat("Spinup test failed - there may be other issues with the model setup\n")
}

# Final summary
if (exists("enhanced_uncertainty_corrected") && enhanced_uncertainty_corrected$n_successful > 10) {
  cat("\n=== SUCCESS! CORRECTED ENHANCED UNCERTAINTY ANALYSIS COMPLETED ===\n")
  cat("Final results: ", enhanced_uncertainty_corrected$n_successful, " successful simulations\n")
  cat("These results can now be used for:\n")
  cat("- Finding optimal parameter combinations\n")
  cat("- Creating uncertainty plots with geom_ribbon\n")
  cat("- Comparing with baseline model\n")
  cat("- Species-specific performance analysis\n")
}
```

# Simple working enhanced simulation function
run_working_enhanced_sims <- function(params, n_sims = 10, effort_scen, 
                                     tolerance = stability_tolerance, 
                                     years = stability_years) {
  
  cat("Running", n_sims, "enhanced simulations with working stability check\n")
  cat("Stability: max", tolerance * 100, "% CV over last", years, "years\n\n")
  ...
  for (i in 1:n_sims) {
    if (i %% 5 == 0) cat("Processing simulation", i, "of", n_sims, "\n")
    
    tryCatch({
      # Create parameter variations
      rand_params <- params
      
      # Catchability variations (moderate)
      gear_df <- gear_params(rand_params)
      if(nrow(gear_df) > 0) {
        for(j in 1:nrow(gear_df)) {
          if(gear_df$catchability[j] > 0) {
            ...
            gear_df$catchability[j] <- gear_df$catchability[j] * exp(noise)
            gear_df$catchability[j] <- min(1, max(0.001, gear_df$catchability[j]))
          }
        }
        gear_params(rand_params) <- gear_df
      }
      
      # Marine mammal abundance variations
      sp_params <- species_params(rand_params)
      marine_mammal_species <- c("minke whales", "orca", "sperm whales", "baleen whales")
      species_indices <- which(sp_params$species %in% marine_mammal_species)
      scaling_factors <- exp(rnorm(length(species_indices), mean = 0, sd = 0.5))
      ...
      # Steady state with reasonable criteria
      steady_params <- steady(rand_params, tol = 0.005, t_max = 800, preserve = c("erepro"))
      
      # Run simulation
      sim <- project(steady_params, t_start = 1841, t_max = 170, effort = effort_scen)
      
      # NO FINAL STABILITY CHECK NEEDED - fishing is supposed to cause variability!
      # We only checked stability during the spinup phase in steady_params
      
      # Calculate performance metrics directly
      mod_yield <- getYield(sim)
      df_mod <- reshape2::melt(mod_yield)
      names(df_mod) <- c("Year", "Species", "Yield")
      
      comparison <- merge(df_mod, yield_ts_tidy,
                         by = c("Year", "Species"),
                         suffixes = c("_mod", "_obs"))
      
      # Calculate weighted RMSE with whale emphasis
      comparison_weighted <- comparison %>%
        left_join(species_weights, by = "Species") %>%
        mutate(weight = ifelse(is.na(weight), 1, weight))
      
      weighted_rmse <- sqrt(weighted.mean((comparison_weighted$Yield_mod - comparison_weighted$Yield_obs)^2,
                                         comparison_weighted$weight, na.rm = TRUE))
      
      sim_results[[length(sim_results) + 1]] <- sim
      performance_results[[length(performance_results) + 1]] <- list(
        weighted_rmse = weighted_rmse
      )
      param_info[[length(param_info) + 1]] <- list(
        sim_id = i,
        catchability = gear_df$catchability,
        marine_mammal_scaling = scaling_factors
      )
  ...
  cat("\nCompleted:", length(sim_results), "successful,", failed_count, "failed\n")
  ...
  return(list(
    ...
    n_successful = length(sim_results),
    n_attempts = n_sims,
    success_rate = length(sim_results) / n_sims * 100
  ))
}

# Run working enhanced simulations
enhanced_results_working <- run_working_enhanced_sims(
  params = params_1841_2010_climate_only,
  n_sims = 25,  # Start with 25 to test
  effort_scen = combined_effort_array
)

# Report results
cat("\n=== WORKING ENHANCED SIMULATION RESULTS ===\n")
cat("Successful simulations:", enhanced_results_working$n_successful, "\n")
cat("Success rate:", round(enhanced_results_working$success_rate, 1), "%\n")

if (enhanced_results_working$n_successful > 0) {
  # Show performance summary
  rmse_values <- sapply(enhanced_results_working$performance, function(x) x$weighted_rmse)
  cv_values <- sapply(enhanced_results_working$performance, function(x) x$max_cv)
  
  cat("Weighted RMSE range:", round(min(rmse_values), 3), "-", round(max(rmse_values), 3), "\n")
  cat("Max CV range:", round(min(cv_values) * 100, 1), "% -", round(max(cv_values) * 100, 1), "%\n")
  
  # Find best simulation
  best_idx <- which.min(rmse_values)
  cat("Best simulation: #", best_idx, "with RMSE =", round(rmse_values[best_idx], 3), "\n")
  
  # Save results
  saveRDS(enhanced_results_working, "enhanced_uncertainty_results_working.RDS")
  
  # If we got good results, run more simulations
  if (enhanced_results_working$success_rate > 50) {
    cat("\nSuccess rate > 50%, running larger batch...\n")
    
    enhanced_results_large <- run_working_enhanced_sims(
      params = params_1841_2010_climate_only,
      n_sims = 100,
      effort_scen = combined_effort_array
    )
    
    cat("Large batch results:", enhanced_results_large$n_successful, "successful of", enhanced_results_large$n_attempts, "\n")
    saveRDS(enhanced_results_large, "enhanced_uncertainty_results_large.RDS")
  }
}
```
...
# If still getting failures, create an even more relaxed version
## Quick Diagnostic Test

Let's first run a small diagnostic test to understand why simulations are failing:

```{r diagnostic-test}
# Simple diagnostic function to test a single simulation
test_single_enhanced_sim <- function(params, effort_scen, verbose = TRUE) {
  
  cat("=== DIAGNOSTIC TEST FOR ENHANCED SIMULATION ===\n")
  
  # Create moderate parameter variations
  rand_params <- params
  
  # Step 1: Moderate catchability changes
  gear_df <- gear_params(rand_params)
  if(nrow(gear_df) > 0) {
    for(j in 1:nrow(gear_df)) {
      if(gear_df$catchability[j] > 0) {
        # Use moderate 20% change instead of random
        gear_df$catchability[j] <- gear_df$catchability[j] * 1.2
      }
    }
    gear_params(rand_params) <- gear_df
    cat("✓ Applied moderate catchability changes\n")
  }
  
  # Step 2: Moderate marine mammal scaling
  sp_params <- species_params(rand_params)
  marine_mammal_species <- c("minke whales", "orca", "sperm whales", "baleen whales")
  species_indices <- which(sp_params$species %in% marine_mammal_species)
  
  # Use moderate 50% increase instead of random
  for (k in seq_along(species_indices)) {
    rand_params@initial_n[species_indices[k],] <-
      rand_params@initial_n[species_indices[k],] * 1.5
  }
  cat("✓ Applied moderate marine mammal abundance scaling\n")
  
  # Step 3: Try steady state with relaxed parameters
  cat("Testing steady state...\n")
  tryCatch({
    steady_params <- steady(rand_params, tol = 0.01, t_max = 500, preserve = c("erepro"))
    cat("✓ Steady state achieved\n")
  }, error = function(e) {
    cat("✗ Steady state failed:", e$message, "\n")
    return(list(success = FALSE, stage = "steady_state", error = e$message))
  })
  ...
  tryCatch({
    ...
    cat("✗ Basic simulation failed:", e$message, "\n")
    return(list(success = FALSE, stage = "simulation", error = e$message))
  })
  
  # Step 5: Test different stability requirements
  cat("Testing stability requirements...\n")
  
  stability_tests <- list(
    list(tolerance = 0.05, years = 50, name = "Original (5% CV, 50 years)"),
    list(tolerance = 0.10, years = 50, name = "Relaxed tolerance (10% CV, 50 years)"),
    list(tolerance = 0.15, years = 30, name = "More relaxed (15% CV, 30 years)"),
    list(tolerance = 0.20, years = 20, name = "Very relaxed (20% CV, 20 years)")
  )
  
  for (test in stability_tests) {
    stability_result <- check_biomass_stability_enhanced(sim, 
                                                        tolerance = test$tolerance, 
                                                        check_years = test$years,
                                                        verbose = TRUE,
                                                        sim_id = "TEST")
    
    if (is.logical(stability_result)) {
      stable <- stability_result
    } else {
      stable <- stability_result$stable
    }
    
    cat("  ", test$name, ":", if (stable) "✓ PASSED" else "✗ FAILED", "\n")
  }
  
  # Get biomass data for manual inspection
  biomass <- getBiomass(sim)
  cat("\nBiomass data summary:\n")
  cat("  Time range:", min(as.numeric(rownames(biomass))), "-", max(as.numeric(rownames(biomass))), "\n")
  cat("  Species:", paste(colnames(biomass), collapse = ", "), "\n")
  
  # Check CV for last 50 years manually
  recent_biomass <- tail(biomass, 50)
  cv_values <- apply(recent_biomass, 2, function(x) {
    if (mean(x) == 0) return(Inf)
    sd(x) / mean(x)
  })
  
  cat("\nCV values for last 50 years:\n")
  for (i in 1:length(cv_values)) {
    cv_pct <- round(cv_values[i] * 100, 1)
    status <- if (cv_values[i] < 0.05) "✓" else if (cv_values[i] < 0.10) "~" else "✗"
    cat("  ", names(cv_values)[i], ":", cv_pct, "%", status, "\n")
  }
...
diagnostic_result <- test_single_enhanced_sim(params_1841_2010_climate_only, combined_effort_array)
```
