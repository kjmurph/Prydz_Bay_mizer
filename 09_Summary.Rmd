...
run_enhanced_single_sim <- function(sim_id, params, catchability_sd, abundance_sd,
  ...
  # Step 5: Validate final simulation stability with detailed diagnostics
  final_stability_check <- check_biomass_stability_enhanced(sim_result$simulation, 
                                                           tolerance = 0.05, 
                                                           check_years = 50,
                                                           verbose = verbose,
                                                           sim_id = sim_id)
  ...
  final_stable <- if (is.logical(final_stability_check)) {
    final_stability_check
  } else {
    final_stability_check$stable
  }
  
  if (!final_stable) {
    failure_details <- if (is.list(final_stability_check)) {
      paste("Worst CV:", round(final_stability_check$worst_cv * 100, 1), 
            "% in", final_stability_check$worst_species,
            "; Stable:", final_stability_check$n_stable, 
            "/ Unstable:", final_stability_check$n_unstable)
    } else {
      "Final biomass not stable"
    }
    ...
    for (years in check_years_options) {
      ...
      for (i in 1:n_tests) {
        ...
        tryCatch({
          ...
          # Check stability using simple approach
          biomass <- getBiomass(sim)
          if (nrow(biomass) >= years) {
            recent_biomass <- tail(biomass, years)
...
## Run Enhanced Analysis with Working Parameters

Now run the enhanced analysis using parameters that actually work:

```{r run-enhanced-with-working-params}
# Use results from the quick test or set reasonable defaults
if (exists("quick_results") && quick_results$success_rate > 0) {
  stability_tolerance <- quick_results$recommended_tolerance
  stability_years <- quick_results$recommended_years
  cat("Using tested parameters: tolerance =", stability_tolerance * 100, "%, years =", stability_years, "\n")
} else {
  # Fallback to very relaxed parameters
  stability_tolerance <- 0.20  # 20% CV
  stability_years <- 20        # 20 years
  cat("Using fallback relaxed parameters: tolerance =", stability_tolerance * 100, "%, years =", stability_years, "\n")
}
...
run_working_enhanced_sims <- function(params, n_sims = 10, effort_scen, 
  ...
  for (i in 1:n_sims) {
    ...
    tryCatch({
      ...
      # Check stability with working parameters
      biomass <- getBiomass(sim)
      if (nrow(biomass) >= years) {
        recent_biomass <- tail(biomass, years)
        cv_values <- apply(recent_biomass, 2, function(x) {
        ...
        is_stable <- all(cv_values < tolerance, na.rm = TRUE)
      } else {
        is_stable <- FALSE
      }
      ...
      if (is_stable) {
        ...
        param_info[[length(param_info) + 1]] <- list(
          ...
          catchability = gear_df$catchability,
          marine_mammal_scaling = scaling_factors,
          worst_cv = max(cv_values, na.rm = TRUE)
        )
      } else {
