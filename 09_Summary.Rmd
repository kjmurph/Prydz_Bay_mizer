...
if (exists("spinup_test_results") && spinup_test_results$success_rate > 0) {
  ...
  enhanced_uncertainty_corrected <- run_enhanced_combined_uncertainty_sims(
    params = params_1841_2010_climate_only,
    n_sims = 100,  # Full simulation set
  ...
  # Save results
  saveRDS(enhanced_uncertainty_corrected, "enhanced_uncertainty_corrected_results.RDS")
  
  # Quick summary
  success_rate <- mean(sapply(enhanced_uncertainty_corrected, function(x) x$success)) * 100
  cat("\n=== ENHANCED UNCERTAINTY ANALYSIS RESULTS ===\n")
  cat("Success rate:", round(success_rate, 1), "%\n")
  
  if (success_rate > 0) {
    cat("SUCCESS! The corrected approach works!\n")
    cat("Key insight validated: Only check stability during spinup (no fishing), not during fished periods\n")
  } else {
    cat("Need further investigation...")
  }
  
} else {
  cat("Spinup test not run or failed - please run the test first\n")
}
```
