...
```{r}
isimip_plankton_exp <- 10^(isimip_plankton)
head(isimip_plankton_exp)
head(sim_original@n_pp[1,])
```

isimip plankton: 1.388271e+27	9.729357e+26	6.818582e+26	4.778636e+26	
sim original:    4.485542e+29 2.140866e+29  1.021796e+29  4.876842e+28 2.327626e+28 1.110933e+28 


```{r}
new_params <- readRDS("params_latest_biomass_09_Aug_2023.RDS")
# new_params # pre-therMizer steady-state
# paramsT # therMizer

# isimip_plankton <- n_pp_array


# n_pp_array <- isimip_plankton

sizes <- names(new_params@initial_n_pp)
...
initialNResource(new_params)[which(w_full(new_params) < 100)]<-10^colMeans(n_pp_array[years_to_use,])[which(w_full(new_params) < 100)]
#need to cut resource at 100g
initialNResource(new_params)[which(w_full(new_params) >=resource_params(new_params)$w_pp_cutoff)]<-0.0 # sets all above 100g to 0
...
8.29e-15     1.19e-14     1.71e-14     2.45e-14     3.51e-14     5.04e-14     7.23e-14     1.04e-13     1.49e-13 
4.485542e+29 2.140866e+29 1.021796e+29 4.876842e+28 2.327626e+28 1.110933e+28 5.302277e+27 2.530679e+27 1.207846e+27 
...
# initialNResource(params)[which(w_full(params) < 100)]<-10^colMeans(n_pp_array[years_to_use,])[which(w_full(params) < resource_params(params)$w_pp_cutoff)]
initialNResource(params)[which(w_full(params) < 100)]<-10^colMeans(n_pp_array[years_to_use,])[which(w_full(params) < 100)]
#need to cut resource at 100g
...
initial_effort(params)<-colMeans(effort[years_to_use,])
...
Plot the initia_n_pp to check it's been updated correctly
Compare to first time step 
units in intial_n_pp are not log values
Values in therMizer are logged, hence the mismatch

initial_n_pp vs sim@n_pp_array[1,] #first time step
initial_

```{r}
sim_original <- project(new_params, t_max = 50)
```


```{r}
head(initialNResource(new_params))
head(initialNResource(paramsT))
```


```{r}
head(sim@n_pp[2,])
head(sim_original@n_pp[2,])

```


```{r}
sim <- project(params_pre, t_max = 50)

plot(sim)
tuneParams(params_pre)
```

```{r}
paramsT_v1 <- steady(paramsT)
paramsT_v1 <- calibrateBiomass(paramsT)

paramsT_v1 <- steady(paramsT_v1)
paramsT_v1 <- matchBiomasses(paramsT_v1)
```
...
# 
# # rownames(ocean_temp) <- time_steps
# # ocean_temp <- ocean_temp[,-1]
# # ocean_temp <- as(ocean_temp, "matrix")
# glimpse(ocean_temp_all)
# head(ocean_temp, 24)
```
...
Set up time steps
```{r}
time_steps <- ocean_temp_tidy$date_tidy
```

Set up sizes
```{r}
sizes <- signif(new_params@w_full, 3)
```
...
# n_pp_array <- read.table("FishMIP_Plankton_Forcing/GFDL_resource_spectra_w_full_164.dat")
n_pp_array <- read.table("FishMIP_Plankton_Forcing/GFDL_resource_spectra_w_full_142.dat")
rownames(n_pp_array) <- time_steps
colnames(n_pp_array) <- sizes
...
# ocean_temp_date <- ocean_temp$date

row.names(ocean_temp) <- ocean_temp_date
...
# ocean_temp_array <- as.matrix(ocean_temp[,2])

row.names(ocean_temp_array) <- time_steps
# 
...
```{r}
rownames(ocean_temp_tidy) <- ocean_temp_tidy$date_tidy

ocean_temp_tos <- ocean_temp_tidy[,2]
names(ocean_temp_tos) <- seq(as.Date("1961-01-01"),as.Date("2010-12-01"),by="months")

ocean_temp_tob <- ocean_temp_tidy[,2]
names(ocean_temp_tob) <- seq(as.Date("1961-01-01"),as.Date("2010-12-01"),by="months")

ocean_temp_final <- abind::abind(ocean_temp_tos,ocean_temp_tob, along = 2)

```


```{r}
# new_params <- setParams(new_params)
# paramsT <- upgradeTherParams(params,
...
paramsT <- upgradeTherParams(new_params,
                             ocean_temp_array = ocean_temp_final,
                             n_pp_array = n_pp_array, 
                             # vertical_migration_array = vertical_migration_array,
                             # exposure_array = exposure_array,
                             aerobic_effect = TRUE, metabolism_effect = TRUE)
...
```{r}
paramsT@other_params$other$n_pp_array
```
...
```{r}
# paramsT <- therMizer::therMizerPredRate(paramsT)

simT <-  project(paramsT,
