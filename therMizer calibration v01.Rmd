...
```{r}
new_params <- readRDS("params_latest_xx.RDS")
...
effort_IWC <- read.csv("FishMIP_fishing_data/IWC_effort.csv")
...
n_pp_array <- read.table("FishMIP_Plankton_Forcing/GFDL_resource_spectra_annual.dat")

rownames(n_pp_array) <- time_steps
sizes <- new_params@w_full
colnames(n_pp_array) <- sizes # signif(sizes, 6)
...
#need to cut resource at 10g
initialNResource(params)[which(w_full(params) >=resource_params(params)$w_pp_cutoff)]<-0.0
