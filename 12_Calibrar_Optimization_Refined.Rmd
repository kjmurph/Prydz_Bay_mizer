...
run_with_enhanced_burnin <- function(params, effort_array, 
                                    ...
                                    t_start = 1841,
                                    sim_years = 170,
                                    tol = 0.005,  # Tighter steady-state tolerance
                                    t_max = 1500,
                                    biomass_tol = 0.075,  # Stricter
...
# Configure optimization method based on parallel availability
optimization_method <- ifelse(use_parallel, "L-BFGS-B", "SANN")
...
opt_result <- tryCatch({
  if (use_parallel) {
    cat("Starting PARALLEL optimization using", n_cores, "cores...\n\n")
    
    # Parallel calibrar optimization
    calibrate(
      par = initial_params,
      fn = calibrar_objective_refined,
      method = "optim",  # Use optim wrapper for parallel
      lower = lower_bounds,
      upper = upper_bounds,
      control = list(
        method = "L-BFGS-B",
        maxit = control_params$maxit,
        factr = control_params$factr,
        pgtol = control_params$pgtol,
        trace = control_params$trace,
        REPORT = control_params$REPORT
      ),
      parallel = TRUE,  # Enable calibrar's parallel processing
    ...
    cat("Starting SEQUENTIAL optimization...\n\n")
    
    # Sequential calibrar optimization
    calibrate(
      par = initial_params,
      fn = calibrar_objective_refined,
...
if (!is.null(opt_result)) {
  ...
  optimization_config <- list(
    ...
    species_weights = species_weights,
    burnin_years = 300,
    biomass_tolerance = 0.075,
    steady_state_tolerance = 0.005,
