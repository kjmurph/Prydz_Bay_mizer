...
# Detect system memory to optimize parallel processing with robust error handling
system_ram_gb <- tryCatch({
  if (.Platform$OS.type == "windows") {
    ...
    # Linux/Mac memory detection
    mem_bytes <- system("free -b | grep '^Mem' | awk '{print $2}'", intern = TRUE)
    if (length(mem_bytes) > 0) {
      mem_gb <- as.numeric(mem_bytes) / 1e9
      if (is.finite(mem_gb) && mem_gb > 0) {
        return(mem_gb)
      }
    }
  ...
  cat("Memory detection failed:", e$message, "\n")
  return(16)  # Conservative default
})

# Ensure system_ram_gb is valid
if (is.na(system_ram_gb) || !is.finite(system_ram_gb) || system_ram_gb <= 0) {
  system_ram_gb <- 16
  cat("Memory detection returned invalid value, using 16GB default\n")
}
...
memory_limited_cores <- max(1, memory_limited_cores)  # Ensure at least 1

n_cores <- min(n_cores, memory_limited_cores, 6) # Cap at 6 for stability
n_cores <- max(1, n_cores)  # Ensure at least 1 core

# Final decision on parallel processing with robust logic
use_parallel <- isTRUE(use_parallel_env) && isTRUE(n_cores > 1) && isTRUE(system_ram_gb >= 8)
...
objective_function_refined <- function(params_vector,
  ...
  if (.GlobalEnv$iter_count %% save_interval == 0) {
    ...
    cat("✓ Checkpoint saved at iteration", .GlobalEnv$iter_count,
...
if (!is.null(sim_optimal)) {
  ...
  if (overall_correlation > 0.8) {
    cat("  ✓ Excellent overall model performance\n")
  } else if (overall_correlation > 0.6) {
    cat("  ≈ Good overall model performance with room for improvement\n")
  } else {
    cat("  ✗ Model performance needs significant improvement\n")
  }
