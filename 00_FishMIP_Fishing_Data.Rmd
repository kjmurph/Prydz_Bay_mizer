...
df_Prydz_catch_prepared <- df_Prydz_catch %>%
  ...
  summarise(catch = sum(FishMIP_yield),
            catch_g = catch*1e6)
...
```{r}
df_IWC_catch <- readRDS("FishMIP_fishing_data/catch_timeseries_BanzareBank_1930_2019_CPUE.rds")
...
```{r}
# Generate a sequence of years to pad the data
filled_years <- full_seq(reordered_df$Year)

# Use complete() to fill missing years and add rows of 0 values
filled_df <- reordered_df %>%
  complete(Year = all_years) %>%
  replace_na(list(across(desired_species_order, ~ 0)))
```
